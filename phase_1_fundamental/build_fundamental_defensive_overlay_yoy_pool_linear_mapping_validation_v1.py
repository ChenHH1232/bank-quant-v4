...
OUTPUT_CONFIG_RESULTS_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_mapping_validation_v1_config_results.csv"
OUTPUT_CHOSEN_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_mapping_validation_v1_chosen.csv"
OUTPUT_SUMMARY_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_mapping_validation_v1.md"
...
FIXED_STOCK_DETERIORATION_THRESHOLD = 0.50
...
MAPPING_SPECS = [
    {"mapping_key": "half", "weight_scale": 0.50, "weight_cap": 1.00},
    {"mapping_key": "scale70", "weight_scale": 0.70, "weight_cap": 1.00},
    {"mapping_key": "full", "weight_scale": 1.00, "weight_cap": 1.00},
    {"mapping_key": "cap40", "weight_scale": 1.00, "weight_cap": 0.40},
]
...
def apply_mapping(signal_value: pd.Series, weight_scale: float, weight_cap: float) -> pd.Series:
    weights = pd.to_numeric(signal_value, errors="coerce") * float(weight_scale)
    return weights.clip(lower=0.0, upper=float(weight_cap))
...
def build_master_frame(
    ...
    for rebalance_date, group in work[eval_mask].groupby("rebalance_date", sort=True):
        ...
        stock_flag = pd.to_numeric(eligible["deterioration_ratio"], errors="coerce") >= float(FIXED_STOCK_DETERIORATION_THRESHOLD)
        row["signal_ratio"] = float(stock_flag.mean()) if len(stock_flag) > 0 else np.nan
        row["deteriorated_count"] = int(stock_flag.sum())
        rows.append(row)
...
def summarize_window(df: pd.DataFrame, mapping_spec: dict[str, object]) -> dict[str, object]:
    work = df.copy()
    work["signal_value"] = pd.to_numeric(work["signal_ratio"], errors="coerce")
    work["defensive_weight"] = apply_mapping(
        work["signal_value"],
        float(mapping_spec["weight_scale"]),
        float(mapping_spec["weight_cap"]),
    )
    ...
    return {
        ...
        "mean_weight_scale": float(
            pd.to_numeric(covered["defensive_weight"], errors="coerce").mean()
            / pd.to_numeric(covered["signal_value"], errors="coerce").replace({0: np.nan}).mean()
        ) if pd.to_numeric(covered["signal_value"], errors="coerce").replace({0: np.nan}).notna().any() else np.nan,
    }
...
def build_config_rows(master_df: pd.DataFrame, fold: dict[str, object]) -> list[dict[str, object]]:
    ...
    for mapping_spec in MAPPING_SPECS:
        window_metrics = {
            window_label: summarize_window(master_df[master_df["window_label"] == window_label].copy(), mapping_spec)
            for window_label in ["train", "test", "review"]
        }
        rows.append(
            {
                "fold_id": fold["fold_id"],
                "config_key": (
                    f"yoy_pool_linear_mapping__thr_{int(round(FIXED_STOCK_DETERIORATION_THRESHOLD * 100))}"
                    f"__{mapping_spec['mapping_key']}__asset_{DEFENSIVE_ASSET_NAME}"
                ),
                "stock_deterioration_threshold": FIXED_STOCK_DETERIORATION_THRESHOLD,
                "mapping_key": mapping_spec["mapping_key"],
                "weight_scale": mapping_spec["weight_scale"],
                "weight_cap": mapping_spec["weight_cap"],
...
def write_summary(config_rows: list[dict[str, object]], chosen_rows: list[dict[str, object]]) -> None:
    ...
    mapping_text = " / ".join(
        f"{item['mapping_key']}(scale={item['weight_scale']:.2f},cap={item['weight_cap']:.2f})"
    ...
    lines = [
        ...
        f"- stock deterioration threshold is fixed at `{int(round(FIXED_STOCK_DETERIORATION_THRESHOLD * 100))}%`",
        ...
        f"- tested treasury mappings = `{mapping_text}`",
    ...
    for _, row in chosen_df.sort_values("fold_id").iterrows():
        lines.append(
            ...
            f"mapping=`{row.get('mapping_key', 'n/a')}` | "
            f"scale=`{row.get('weight_scale', 'n/a')}` | "
            f"cap=`{row.get('weight_cap', 'n/a')}` | "
