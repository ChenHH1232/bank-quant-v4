...
OUTPUT_CONFIG_RESULTS_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_persist_rolling_validation_v3_config_results.csv"
OUTPUT_CHOSEN_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_persist_rolling_validation_v3_chosen.csv"
OUTPUT_SUMMARY_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_persist_rolling_validation_v3.md"
...
def summarize_window(df: pd.DataFrame, stock_threshold: float) -> dict[str, object]:
    ...
    work["is_bad_state"] = work["signal_value"] > 0
    streaks: list[int] = []
    current_streak = 0
    for is_bad in work["is_bad_state"].fillna(False).tolist():
        if is_bad:
            current_streak += 1
        else:
            current_streak = 0
        streaks.append(current_streak)
    work["bad_streak"] = streaks
    work["persistence_add"] = np.where(
        work["bad_streak"] >= 3,
        PERSISTENCE_ADD_3P,
        np.where(work["bad_streak"] >= 2, PERSISTENCE_ADD_2, 0.0),
    )
    work["defensive_weight"] = (work["signal_value"] + work["persistence_add"]).clip(lower=0.0, upper=DEFENSIVE_WEIGHT_CAP)
