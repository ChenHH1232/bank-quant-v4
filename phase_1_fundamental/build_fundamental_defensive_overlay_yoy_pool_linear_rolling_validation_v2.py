...
OUTPUT_CONFIG_RESULTS_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_rolling_validation_v2_config_results.csv"
OUTPUT_CHOSEN_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_rolling_validation_v2_chosen.csv"
OUTPUT_SUMMARY_PATH = SCRIPT_DIR / "fundamental_defensive_overlay_yoy_pool_linear_rolling_validation_v2.md"
...
STOCK_DETERIORATION_THRESHOLDS = [0.40, 0.50, 0.60]
...
def summarize_window(df: pd.DataFrame, stock_threshold: float) -> dict[str, object]:
    ...
    work["defensive_weight"] = work["signal_value"].clip(lower=0.0, upper=1.0)
...
def build_config_rows(master_df: pd.DataFrame, fold: dict[str, object]) -> list[dict[str, object]]:
    ...
    for threshold in STOCK_DETERIORATION_THRESHOLDS:
        ...
        rows.append(
            {
                "fold_id": fold["fold_id"],
                "config_key": f"yoy_pool_linear__thr_{int(round(threshold * 100))}__asset_{DEFENSIVE_ASSET_NAME}",
