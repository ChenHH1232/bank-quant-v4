...
OUTPUT_DETAIL_PATH = SCRIPT_DIR / "fundamental_mean_reversion_entry_rolling_validation_v2_detail.csv"
OUTPUT_SUMMARY_PATH = SCRIPT_DIR / "fundamental_mean_reversion_entry_rolling_validation_v2_summary.csv"
OUTPUT_NOTE_PATH = SCRIPT_DIR / "fundamental_mean_reversion_entry_rolling_validation_v2.md"
...
def basket_signal(
    ...
    cross = cross[(cross["mean_reversion_ready_v1"] == 1) & cross[signal_col].notna()]
    if cross.empty:
        return math.nan
    return float(pd.to_numeric(cross[signal_col], errors="coerce").mean())
    ...
    for row in holdings_rows:
        ...
        for offset in range(SEARCH_TRADING_DAYS):
            ...
            if signal_date is None:
                break
            signal_value = basket_signal(daily_df, codes, signal_date, signal_col)
...
def choose_reversion_entry_date(
    ...
    for idx in range(start_idx, end_idx + 1):
        ...
        if (not lower_is_better) and signal_value >= threshold:
            return signal_date, signal_value, idx - start_idx
    forced_signal = basket_signal(daily_df, codes, forced_date, signal_col)
...
def build_detail_rows() -> list[dict[str, object]]:
    ...
    for fold in folds:
        ...
        train_holdings = build_window_holdings(scored_df, train_start, test_start)
        ...
        for signal_name, spec in SIGNAL_SPECS.items():
            ...
            for threshold_quantile in THRESHOLD_QUANTILES:
                threshold_map[(signal_name, threshold_quantile)] = float(
                    train_signal_series.quantile(threshold_quantile)
                )
                        ...
                        detail_rows.append(
                            {
                                ...
                                "baseline_full_entry_return": round(full_return, 6)
                                if not math.isnan(full_return)
                                else math.nan,
                    ...
                    rows.append(
                        {
                            ...
                            if not math.isnan(stats["cum_return"])
                            else math.nan,
                            "mean_return": round(stats["mean_return"], 6)
                            if not math.isnan(stats["mean_return"])
                            else math.nan,
...
def write_note(summary_rows: list[dict[str, object]]) -> None:
    ...
    if summary_df.empty:
        ...
        review_df = review_df.sort_values(
            ["strategy_name", "cum_return", "max_drawdown"],
            ascending=[True, False, False],
        )
        for _, row in review_df.iterrows():
            lines.append(
                f"- `{row['signal_name']}` q=`{int(float(row['threshold_quantile']) * 100)}%` `{row['window_label']}` `{row['strategy_name']}` | "
                f"periods=`{row['period_count']}` | cum_return=`{row['cum_return']}` | mean_return=`{row['mean_return']}` | "
