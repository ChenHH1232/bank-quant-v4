...
OUTPUT_CONFIG_RESULTS_PATH = SCRIPT_DIR / "pure_fundamental_execution_staggered_rolling_validation_v1_config_results.csv"
OUTPUT_CHOSEN_PATH = SCRIPT_DIR / "pure_fundamental_execution_staggered_rolling_validation_v1_chosen.csv"
OUTPUT_SUMMARY_PATH = SCRIPT_DIR / "pure_fundamental_execution_staggered_rolling_validation_v1.md"

TARGET_COMBO = "combo__ic_weight_train"
BASELINE_CONFIG_KEY = "pf_stagger__top_08__imm_100__delay_00"

HOLD_COUNT = 8
STAGGER_IMMEDIATE_WEIGHTS = [1.0, 0.7, 0.5]
STAGGER_DELAY_DAYS = [0, 3, 5]
...
def build_strategy_key(config: dict[str, float]) -> str:
    return "pf_stagger__top_%02d__imm_%03d__delay_%02d" % (
        int(config["hold_count"]),
        int(round(config["immediate_weight"] * 100)),
...
def simulate_period_return(
    ...
    for code in top_codes:
        ...
        if stock_df.empty:
            continue
        first_entry_row = get_delay_entry_row(stock_df, rebalance_date, 0)
        second_entry_row = get_delay_entry_row(stock_df, rebalance_date, int(config["delay_days"]))
        end_row = get_aligned_row(stock_df, next_rebalance_date, "end")
        if first_entry_row is None or end_row is None:
            continue
        first_entry_day = first_entry_row["day"]
        second_entry_day = second_entry_row["day"] if second_entry_row is not None else first_entry_day
        end_day = end_row["day"]
        if first_entry_day >= end_day:
            continue
        window = stock_df[(stock_df["day"] > first_entry_day) & (stock_df["day"] <= end_day)].copy()
        if window.empty:
            continue
        immediate_weight = float(config["immediate_weight"])
        delayed_weight = 1.0 - immediate_weight
        stock_windows[code] = {
            row["day"]: (
                float(row["daily_return"]) if pd.notna(row["daily_return"]) else np.nan,
                immediate_weight if row["day"] <= second_entry_day else 1.0,
                delayed_weight if row["day"] <= second_entry_day else 0.0,
            )
            for _, row in window.iterrows()
        }
        entry_days[code] = second_entry_day
    ...
    for day in ordered_days:
        ...
        for code, day_map in stock_windows.items():
            ...
            if payload is None:
                continue
            daily_return, active_multiplier, _ = payload
            if pd.notna(daily_return):
                portfolio_return += base_weight * float(active_multiplier) * float(daily_return)
            active_weight_sum += base_weight * float(active_multiplier)
...
def build_master_frame(
    ...
    for idx, rebalance_date in enumerate(rebalance_dates):
        ...
        for immediate_weight in STAGGER_IMMEDIATE_WEIGHTS:
            for delay_days in STAGGER_DELAY_DAYS:
                if immediate_weight >= 0.999 and delay_days != 0:
                    continue
                if delay_days == 0 and immediate_weight < 0.999:
                    continue
                config = {
                    "hold_count": HOLD_COUNT,
                    "immediate_weight": immediate_weight,
...
def write_summary(config_rows: list[dict[str, object]], chosen_rows: list[dict[str, object]]) -> None:
    ...
    for _, row in chosen_df.sort_values("fold_id").iterrows():
        lines.append(
            ...
            f"hold_count=`{row.get('hold_count', 'n/a')}` | immediate_weight=`{row.get('immediate_weight', 'n/a')}` | delay_days=`{row.get('delay_days', 'n/a')}` | "
    ...
    if not candidate_df.empty:
        summary_df = (
            candidate_df.groupby(["config_key", "hold_count", "immediate_weight", "delay_days"], dropna=False)
            .agg(
                folds=("fold_id", "count"),
