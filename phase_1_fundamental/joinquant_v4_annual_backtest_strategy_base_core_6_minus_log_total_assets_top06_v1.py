...
def initialize(context):
    ...
    g.group_count = 5
    g.hold_bucket = 5
    g.target_hold_count = 6
    # base_core_6 candidate = base_core_7 minus derived__log_total_assets
    # execution candidate = direct top-6 equal weight, aligned with local robustness validation
...
def do_rebalance(context):
    ...
    long_df = score_df.head(int(g.target_hold_count)).copy()
    if long_df.empty:
        log.info('No stocks entered direct top-%d selection on %s' % (int(g.target_hold_count), str(factor_date)))
        return
    ...
    if g.debug:
        ...
        log.info('selected_count=%d target_hold_count=%d long_list=%s' % (
            len(long_list),
            int(g.target_hold_count),
            str(long_list),
        ))
