...
def initialize(context):
    ...
    g.group_count = 5
    g.hold_bucket = 5
    # base_core_6 candidate = base_core_7 minus derived__log_total_assets
    # price-overheat candidate = cap only stocks that run hot without enough fundamental improvement
    g.strategy_line = 'benchmark'  # 'primary' or 'benchmark'
    g.debug = True
    g.overheat_trigger_growth = 0.20
    g.overheat_cap_weight = 0.10
    g.max_improve_count = 1
    g.min_available_improvement_signals = 3
    ...
    run_daily(apply_overheat_caps, time='14:30')
...
def do_rebalance(context):
    ...
    overheat_state = build_overheat_state(long_df, factor_date, target_weight)
    factor_summary = summarize_factor_specs(factor_specs)
    top_score_preview = build_score_preview(score_df.head(10))
    long_score_preview = build_score_preview(
        long_df[['code', 'final_score', 'factor_count', 'fund_improve_count', 'overheat_eligible_flag']].head(20)
    )
    ...
    if g.debug:
        ...
        log.info('price_overheat_state=%s' % str(overheat_state))
    ...
    g.overheat_state = overheat_state
    ...
    g.plan_usage_log.append({
        ...
        'overheat_state': str(overheat_state),
...
def build_score_frame(context, stocks, factor_date, factor_specs):
    ...
    score_df['indicator__roe'] = current_snapshot.get('indicator__roe')
    score_df['indicator__eps'] = current_snapshot.get('indicator__eps')
    score_df['prev__indicator__roe'] = previous_snapshot.get('indicator__roe')
    score_df['prev__indicator__eps'] = previous_snapshot.get('indicator__eps')
    score_df['bank_indicator__Nonperforming_loan_rate'] = current_bank.get('bank_indicator__Nonperforming_loan_rate')
    score_df['bank_indicator__non_performing_loan_provision_coverage'] = current_bank.get('bank_indicator__non_performing_loan_provision_coverage')
    score_df['bank_indicator__deposit_loan_ratio'] = current_bank.get('bank_indicator__deposit_loan_ratio')
    score_df['bank_indicator__capital_adequacy_ratio'] = current_bank.get('bank_indicator__capital_adequacy_ratio')
    score_df['prev__bank_indicator__Nonperforming_loan_rate'] = previous_bank.get('bank_indicator__Nonperforming_loan_rate')
    score_df['prev__bank_indicator__non_performing_loan_provision_coverage'] = previous_bank.get('bank_indicator__non_performing_loan_provision_coverage')
    score_df['prev__bank_indicator__deposit_loan_ratio'] = previous_bank.get('bank_indicator__deposit_loan_ratio')
    score_df['prev__bank_indicator__capital_adequacy_ratio'] = previous_bank.get('bank_indicator__capital_adequacy_ratio')
    ...
    score_df = add_fundamental_improvement_columns(score_df)
    return score_df


def add_fundamental_improvement_columns(score_df):
    ...
    for _, row in work_df.iterrows():
        ...
        for factor_name, direction in [
            ...
            previous_value = pd.to_numeric(pd.Series([row.get('prev__' + factor_name)]), errors='coerce').iloc[0]
            ...
            if direction == 'higher' and delta > 0:
                ...
                improve_count += 1
        improve_counts.append(float(improve_count))
        available_counts.append(float(available_count))
    work_df['fund_improve_count'] = improve_counts
    work_df['fund_improve_available_count'] = available_counts
    work_df['overheat_eligible_flag'] = (
        (pd.to_numeric(work_df['fund_improve_available_count'], errors='coerce') >= float(g.min_available_improvement_signals))
        & (pd.to_numeric(work_df['fund_improve_count'], errors='coerce') <= float(g.max_improve_count))
    ).astype(int)
    if g.debug:
        eligible_count = int(work_df['overheat_eligible_flag'].sum())
        lines = []
        for _, row in work_df.sort_values(['final_score', 'code'], ascending=[False, True]).head(12).iterrows():
            lines.append(
                '%s:score=%.4f,improve=%s,avail=%s,eligible=%s' % (
                    row['code'],
                    float(row['final_score']) if pd.notna(row.get('final_score')) else float('nan'),
                    str(int(row['fund_improve_count'])) if pd.notna(row.get('fund_improve_count')) else 'na',
                    str(int(row['fund_improve_available_count'])) if pd.notna(row.get('fund_improve_available_count')) else 'na',
                    str(int(row['overheat_eligible_flag'])) if pd.notna(row.get('overheat_eligible_flag')) else 'na',
                )
            )
        log.info(
            'fund_improve_summary eligible=%d/%d threshold<=%d min_avail=%d preview=%s' % (
                eligible_count,
                len(work_df),
                int(g.max_improve_count),
                int(g.min_available_improvement_signals),
                '[' + ' ; '.join(lines) + ']',
            )
        )
    return work_df
...
def build_overheat_state(long_df, factor_date, target_weight):
    state = {}
    if long_df is None or len(long_df) == 0:
        return state
    price_map = fetch_price_snapshot(long_df['code'].tolist(), factor_date=factor_date, use_current=False)
    for _, row in long_df.iterrows():
        code = str(row['code'])
        entry_price = pd.to_numeric(pd.Series([price_map.get(code)]), errors='coerce').iloc[0]
        state[code] = {
            'entry_price': float(entry_price) if pd.notna(entry_price) else None,
            'base_target_weight': float(target_weight),
            'current_target_weight': float(target_weight),
            'fund_improve_count': float(pd.to_numeric(pd.Series([row.get('fund_improve_count')]), errors='coerce').iloc[0]) if pd.notna(pd.to_numeric(pd.Series([row.get('fund_improve_count')]), errors='coerce').iloc[0]) else None,
            'fund_improve_available_count': float(pd.to_numeric(pd.Series([row.get('fund_improve_available_count')]), errors='coerce').iloc[0]) if pd.notna(pd.to_numeric(pd.Series([row.get('fund_improve_available_count')]), errors='coerce').iloc[0]) else None,
            'overheat_eligible_flag': int(row.get('overheat_eligible_flag', 0)),
            'trimmed_flag': 0,
        }
    if g.debug:
        preview = []
        ...
        for code in sorted(state.keys()):
            item = state[code]
            if int(item.get('overheat_eligible_flag', 0)) == 1:
                eligible_count += 1
            if item.get('entry_price') is None:
                missing_price_count += 1
            preview.append(
                '%s:entry_price=%s,base_w=%.4f,improve=%s,avail=%s,eligible=%d' % (
                    code,
                    ('%.4f' % float(item['entry_price'])) if item.get('entry_price') is not None else 'na',
                    float(item.get('base_target_weight', 0.0)),
                    str(item.get('fund_improve_count')),
                    str(item.get('fund_improve_available_count')),
                    int(item.get('overheat_eligible_flag', 0)),
                )
            )
        log.info(
            'price_overheat_state_built factor_date=%s eligible=%d/%d missing_entry_price=%d preview=%s' % (
                str(factor_date),
                eligible_count,
                len(state),
                missing_price_count,
...
def fetch_price_snapshot(stocks, factor_date=None, use_current=False):
    result = {}
    if not stocks:
        return result
    if use_current:
        try:
            ...
            log.info('get_current_data failed %s' % str(exc))
            return result
        for code in stocks:
            try:
                ...
                price = np.nan
            result[str(code)] = float(price) if pd.notna(price) else np.nan
        return result
    ...
    for code in stocks:
        try:
            ...
            log.info('get_price failed code=%s date=%s %s' % (str(code), str(factor_date), str(exc)))
            result[str(code)] = np.nan
            continue
        close_value = np.nan
        try:
            if price_df is not None and len(price_df) > 0:
                close_value = pd.to_numeric(pd.Series([price_df['close'].iloc[-1]]), errors='coerce').iloc[0]
        except Exception:
            close_value = np.nan
        result[str(code)] = float(close_value) if pd.notna(close_value) else np.nan
    return result


def apply_overheat_caps(context):
    if not g.overheat_state:
        if g.debug:
            log.info('apply_overheat_caps skipped because g.overheat_state is empty')
        return
    tracked_codes = list(g.overheat_state.keys())
    current_codes = [code for code in tracked_codes if code in context.portfolio.positions]
    if not current_codes:
        if g.debug:
            log.info('apply_overheat_caps skipped because no tracked codes are currently held')
        return
    current_date = context.current_dt.date()
    price_map = fetch_price_snapshot(current_codes, factor_date=current_date, use_current=True)
    if g.debug:
        log.info(
            'apply_overheat_caps current_date=%s tracked=%d held=%d trigger_growth=%.4f cap_weight=%.4f' % (
                str(current_date),
                len(tracked_codes),
                len(current_codes),
                float(g.overheat_trigger_growth),
                float(g.overheat_cap_weight),
            )
        )
        preview = []
        for code in current_codes[:20]:
            state = g.overheat_state.get(code, {})
            preview.append(
                '%s:entry_price=%s,current_price=%s,eligible=%d,current_w=%.4f' % (
                    code,
                    ('%.4f' % float(state['entry_price'])) if state.get('entry_price') is not None else 'na',
                    ('%.4f' % float(price_map[code])) if code in price_map and pd.notna(price_map[code]) else 'na',
                    int(state.get('overheat_eligible_flag', 0)),
                    float(state.get('current_target_weight', 0.0)),
                )
            )
        log.info('apply_overheat_caps price_preview=%s' % ('[' + ' ; '.join(preview) + ']'))
    ...
    for code in current_codes:
        checked_count += 1
        state = g.overheat_state.get(code, {})
        if int(state.get('overheat_eligible_flag', 0)) != 1:
            blocked_count += 1
            continue
        eligible_count += 1
        entry_price = state.get('entry_price')
        current_price = price_map.get(code)
        if entry_price is None or current_price is None or pd.isna(current_price) or float(entry_price) <= 0:
            if g.debug:
                log.info(
                    'overheat_check_missing_data code=%s entry_price=%s current_price=%s' % (
                        code,
                        str(entry_price),
                        str(current_price),
                    )
            ...
            continue
        growth = float(current_price) / float(entry_price) - 1.0
        if g.debug:
            log.info(
                'overheat_check code=%s growth=%.4f trigger=%.4f improve_count=%s avail=%s eligible=%d current_w=%.4f' % (
                    code,
                    growth,
                    float(g.overheat_trigger_growth),
                    str(state.get('fund_improve_count')),
                    str(state.get('fund_improve_available_count')),
                    int(state.get('overheat_eligible_flag', 0)),
                    float(state.get('current_target_weight', state.get('base_target_weight', 0.0))),
                )
            )
        if growth < float(g.overheat_trigger_growth):
            continue
        triggered_count += 1
        old_weight = float(state.get('current_target_weight', state.get('base_target_weight', 0.0)))
        new_weight = min(old_weight, float(g.overheat_cap_weight))
        if new_weight >= old_weight - 1e-12:
            if g.debug:
                log.info(
                    'overheat_trigger_no_trim code=%s growth=%.4f old_weight=%.4f cap_weight=%.4f' % (
                        code,
                        growth,
                        old_weight,
                        float(g.overheat_cap_weight),
            ...
            continue
        target_value = context.portfolio.total_value * new_weight
        try:
            order_target_value(code, target_value)
            state['current_target_weight'] = float(new_weight)
            state['trimmed_flag'] = 1
            trimmed_count += 1
            if g.debug:
                log.info(
                    'overheat trim code=%s growth=%.4f old_weight=%.4f new_weight=%.4f improve_count=%s available=%s' % (
                        code,
                        growth,
                        old_weight,
                        new_weight,
            ...
            log.info('overheat trim failed %s %s' % (code, str(exc)))
    if g.debug:
        log.info(
            'apply_overheat_caps summary checked=%d eligible=%d triggered=%d trimmed=%d blocked=%d' % (
                checked_count,
                eligible_count,
                triggered_count,
                trimmed_count,
