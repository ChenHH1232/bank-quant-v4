...
def initialize(context):
    ...
    # Freeze the local rolling candidate:
    # bank_core_3 trigger + warning/severe thresholds 40% / 55%
    # + equity mapping 60% / 40% + defensive sleeve held as cash.
    ...
    g.use_cash_defensive_sleeve = True
...
def do_rebalance(context):
    ...
    if g.debug:
        ...
        log.info(
            'fundamental_regime=%s equity_weight=%.2f cash_weight=%.2f summary=%s' % (
    ...
    g.plan_usage_log.append({
        ...
        'cash_weight': round(defensive_weight, 4),
