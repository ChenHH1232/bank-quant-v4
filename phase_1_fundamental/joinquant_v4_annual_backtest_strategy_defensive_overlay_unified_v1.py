...
def initialize(context):
    ...
    # Main switchboard: change names or logic here without touching the core flow.
    g.overlay_variant = 'shortbond_6040_4055'
    overlay_cfg = dict(OVERLAY_VARIANT_LIBRARY[g.overlay_variant])
    g.regime_metric_names = ['npl', 'capital', 'coverage']
    g.regime_warning_threshold = overlay_cfg['warning_threshold']
    g.regime_severe_threshold = overlay_cfg['severe_threshold']
    g.regime_warning_equity_weight = overlay_cfg['warning_equity_weight']
    g.regime_severe_equity_weight = overlay_cfg['severe_equity_weight']
    g.defensive_asset = overlay_cfg['defensive_asset']
...
def evaluate_fundamental_regime(factor_date):
    ...
    metrics = {}
    if 'npl' in g.regime_metric_names:
        metrics['npl_deterioration_ratio'] = metric_library['npl_deterioration_ratio']
    if 'capital' in g.regime_metric_names:
        metrics['capital_deterioration_ratio'] = metric_library['capital_deterioration_ratio']
    if 'coverage' in g.regime_metric_names:
        metrics['coverage_deterioration_ratio'] = metric_library['coverage_deterioration_ratio']
