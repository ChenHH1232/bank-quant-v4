...
def initialize(context):
    ...
    g.group_count = 5
    g.hold_bucket = 5
    g.strategy_line = 'primary'  # 'primary' or 'benchmark'
    g.debug = True
    g.enable_fundamental_defensive_overlay = True
    g.defensive_asset = '511010.XSHG'
    g.defense_stock_deterioration_threshold = 0.50
    g.defense_min_valid_factor_count = 2
    g.pool_filter_lookback_days = 20
    g.pool_filter_keep_ratio = 0.80
    g.defense_weight_scale = 0.70
    g.defense_weight_cap = 1.00
...
def do_rebalance(context):
    ...
    defense_state = build_fundamental_defense_state(context, tradable, factor_date, factor_specs, score_df)
    defensive_weight = float(defense_state.get('defensive_weight', 0.0))
    defensive_weight = max(0.0, min(1.0, defensive_weight))
    stock_weight_total = max(0.0, 1.0 - defensive_weight)
    target_weight = stock_weight_total / float(len(long_list))
    ...
    if g.debug:
        ...
        log.info('defense_state=%s' % str(defense_state))
    ...
    g.plan_usage_log.append({
        ...
        'defensive_asset': g.defensive_asset,
        'defensive_weight': defensive_weight,
        'deteriorated_stock_ratio': defense_state.get('deteriorated_stock_ratio'),
        'eligible_stock_count': defense_state.get('eligible_stock_count'),
        'pool_stock_count': defense_state.get('pool_stock_count'),
...
def build_score_frame(context, stocks, factor_date, factor_specs):
    score_df = build_raw_factor_frame(stocks, factor_date, factor_specs)
    if score_df.empty:
        return pd.DataFrame()

    for spec in factor_specs:
        factor_name = spec['name']
        if factor_name not in score_df.columns:
            continue
        z_col = 'z__' + factor_name
        adj_col = 'adj__' + factor_name
        score_df[z_col] = zscore_series(winsorize_series(score_df[factor_name]))
...
def build_raw_factor_frame(stocks, factor_date, factor_specs):
...
def build_fundamental_defense_state(context, tradable, factor_date, factor_specs, score_df):
    state = {
        'enabled': bool(getattr(g, 'enable_fundamental_defensive_overlay', False)),
        'threshold': float(getattr(g, 'defense_stock_deterioration_threshold', 0.50)),
        'defensive_weight': 0.0,
        'deteriorated_stock_ratio': 0.0,
    ...
    pool_filter_df = fetch_pool_filter_frame(tradable, factor_date)
    pool_df = score_df[['code']].copy()
    pool_df = pool_df.merge(pool_filter_df, on='code', how='left')
    if 'liquidity_top_80_flag' in pool_df.columns:
        pool_df = pool_df[pool_df['liquidity_top_80_flag'] == 1].copy()
    if 'market_cap_top_80_flag' in pool_df.columns:
        pool_df = pool_df[pool_df['market_cap_top_80_flag'] == 1].copy()
    if pool_df.empty:
        pool_df = score_df[['code']].copy()

    pool_codes = pool_df['code'].tolist()
    state['pool_stock_count'] = len(pool_codes)
    if not pool_codes:
        return state

    current_raw_df = build_raw_factor_frame(pool_codes, factor_date, factor_specs)
    yoy_factor_date = get_yoy_snapshot_factor_date(factor_date)
    prev_raw_df = build_raw_factor_frame(pool_codes, yoy_factor_date, factor_specs) if yoy_factor_date else pd.DataFrame()
    if current_raw_df.empty or prev_raw_df.empty:
        return state

    work = current_raw_df[['code']].copy()
    direction_map = {spec['name']: int(spec['direction_sign']) for spec in factor_specs}
    valid_factor_names = [spec['name'] for spec in factor_specs if spec['name'] in current_raw_df.columns and spec['name'] in prev_raw_df.columns]
    if not valid_factor_names:
        return state

    for factor_name in valid_factor_names:
        current = pd.to_numeric(current_raw_df[factor_name], errors='coerce')
        prev = pd.to_numeric(prev_raw_df[factor_name], errors='coerce')
        if direction_map[factor_name] > 0:
            flag = np.where(current.notna() & prev.notna(), (current < prev).astype(float), np.nan)
        else:
            flag = np.where(current.notna() & prev.notna(), (current > prev).astype(float), np.nan)
        work['flag__' + factor_name] = flag

    flag_cols = ['flag__' + name for name in valid_factor_names]
    flag_frame = work[flag_cols].apply(pd.to_numeric, errors='coerce')
    work['valid_factor_count'] = flag_frame.notna().sum(axis=1)
    work['deteriorated_factor_count'] = flag_frame.fillna(0.0).sum(axis=1)
    work['deterioration_ratio'] = work['deteriorated_factor_count'] / work['valid_factor_count'].replace({0: np.nan})

    min_required = min(int(getattr(g, 'defense_min_valid_factor_count', 2)), len(valid_factor_names))
    eligible = work[work['valid_factor_count'] >= min_required].copy()
    state['eligible_stock_count'] = int(len(eligible))
    if eligible.empty:
        return state

    stock_flag = pd.to_numeric(eligible['deterioration_ratio'], errors='coerce') >= state['threshold']
    deteriorated_stock_ratio = float(stock_flag.mean()) if len(stock_flag) > 0 else 0.0
    state['deteriorated_stock_ratio'] = deteriorated_stock_ratio
    scaled_weight = deteriorated_stock_ratio * float(getattr(g, 'defense_weight_scale', 1.0))
    capped_weight = min(float(getattr(g, 'defense_weight_cap', 1.0)), scaled_weight)
    state['defensive_weight'] = max(0.0, min(1.0, capped_weight))
    return state


def fetch_pool_filter_frame(stocks, factor_date):
    out = pd.DataFrame({'code': list(stocks)})
    out['liquidity_top_80_flag'] = 1
    out['market_cap_top_80_flag'] = 1
    ...
    try:
        price_df = get_price(
            ...
            frequency='daily',
            fields=['money'],
            count=int(getattr(g, 'pool_filter_lookback_days', 20)),
            skip_paused=False,
            fq='post',
            panel=False,
        )
        if price_df is not None and len(price_df) > 0:
            price_df = price_df[['code', 'money']].copy()
            price_df['money'] = pd.to_numeric(price_df['money'], errors='coerce')
            liquidity_df = (
                price_df.dropna(subset=['code', 'money'])
                .groupby('code', as_index=False)['money']
                .mean()
                .rename(columns={'money': 'avg_money_20d'})
            )
            out = out.merge(liquidity_df, on='code', how='left')
            liquidity_keep = max(1, int(np.ceil(len(out) * float(getattr(g, 'pool_filter_keep_ratio', 0.80)))))
            liquidity_codes = (
                out.dropna(subset=['avg_money_20d'])
                .sort_values(['avg_money_20d', 'code'], ascending=[False, True])
                .head(liquidity_keep)['code']
                .tolist()
            )
            if liquidity_codes:
                out['liquidity_top_80_flag'] = out['code'].isin(liquidity_codes).astype(int)
    except Exception as exc:
        if g.debug:
            log.info('pool liquidity filter fallback: %s' % str(exc))
    ...
    try:
        ...
        if valuation_df is not None and len(valuation_df) > 0:
            valuation_df = valuation_df[['code', 'market_cap']].copy()
            valuation_df['market_cap'] = pd.to_numeric(valuation_df['market_cap'], errors='coerce')
            out = out.drop(columns=['market_cap'], errors='ignore').merge(valuation_df, on='code', how='left')
            market_cap_keep = max(1, int(np.ceil(len(out) * float(getattr(g, 'pool_filter_keep_ratio', 0.80)))))
            market_cap_codes = (
                out.dropna(subset=['market_cap'])
                .sort_values(['market_cap', 'code'], ascending=[False, True])
                .head(market_cap_keep)['code']
                .tolist()
            )
            if market_cap_codes:
                out['market_cap_top_80_flag'] = out['code'].isin(market_cap_codes).astype(int)
    except Exception as exc:
        if g.debug:
            log.info('pool market-cap filter fallback: %s' % str(exc))
...
def execute_target_weights(context, long_list, target_weight, defensive_asset, defensive_weight):
    ...
    if defensive_asset is not None:
        target_value_map[defensive_asset] = context.portfolio.total_value * max(0.0, float(defensive_weight))
