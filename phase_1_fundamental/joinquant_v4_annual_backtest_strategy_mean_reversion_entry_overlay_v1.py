...
def initialize(context):
    ...
    g.entry_overlay_mode = 'split_50_50_reversion'
    g.entry_signal_name = 'rev_5d'      # 'rev_5d' or 'rev5_abnvol'
    g.entry_threshold_label = 'q30'     # 'q20' / 'q30' / 'q40'
...
def build_entry_trigger_library():
    return {
        'annual_01': {
            'rev5_abnvol': {'q20': 0.192171, 'q30': 0.212083, 'q40': 0.230121},
            'rev_5d': {'q20': -0.017025, 'q30': -0.010794, 'q40': -0.005379},
        },
        'annual_02': {
            'rev5_abnvol': {'q20': 0.197893, 'q30': 0.215113, 'q40': 0.237713},
            'rev_5d': {'q20': -0.019196, 'q30': -0.011927, 'q40': -0.006048},
        },
        'annual_03': {
            'rev5_abnvol': {'q20': 0.211723, 'q30': 0.228052, 'q40': 0.245988},
            'rev_5d': {'q20': -0.016045, 'q30': -0.008768, 'q40': -0.003966},
        },
        'annual_04': {
            'rev5_abnvol': {'q20': 0.217131, 'q30': 0.235186, 'q40': 0.249745},
            'rev_5d': {'q20': -0.014600, 'q30': -0.008150, 'q40': -0.004018},
        },
        'annual_05': {
            'rev5_abnvol': {'q20': 0.214904, 'q30': 0.232579, 'q40': 0.246962},
            'rev_5d': {'q20': -0.015054, 'q30': -0.007976, 'q40': -0.003927},
        },
    }
...
def do_rebalance(context):
    ...
    entry_threshold_value = get_entry_trigger_threshold(plan['plan_id'])

    if g.debug:
        log.info('========== V4 annual rebalance ==========')
        log.info('current_date=%s factor_date=%s plan=%s line=%s mode=%s' % (
        ...
        log.info('entry_signal=%s threshold_label=%s threshold_value=%s' % (
            str(g.entry_signal_name),
            str(g.entry_threshold_label),
            str(safe_round(entry_threshold_value, 6)),
        ))

    apply_entry_overlay(context, plan, factor_date, long_list, target_weight)
    ...
    g.plan_usage_log.append({
        ...
        'entry_overlay_mode': g.entry_overlay_mode,
        'entry_signal_name': g.entry_signal_name,
        'entry_threshold_label': g.entry_threshold_label,
...
def apply_entry_overlay(context, plan, factor_date, long_list, target_weight):
    ...
    initial_ratio = safe_float(getattr(g, 'entry_initial_ratio', 0.5))
    initial_ratio = min(max(initial_ratio, 0.0), 1.0)
    execute_target_ratio(context, long_list, initial_ratio)

    threshold_value = get_entry_trigger_threshold(plan['plan_id'])
    ...
    g.pending_entry = {
        ...
        'signal_name': g.entry_signal_name,
        'threshold_label': g.entry_threshold_label,
        'threshold_value': threshold_value,
        'initial_signal': initial_signal,
        'completed': False,
    }
    if g.debug:
        log.info('entry_overlay_activated mode=%s initial_ratio=%.4f signal=%s threshold=%s initial_signal=%s' % (
            mode,
            initial_ratio,
            str(g.entry_signal_name),
...
def maybe_complete_pending_entry(context):
    ...
    approved_codes = get_tradable_stocks(context, pending.get('target_codes', []))
    ...
    if mode == 'split_50_50_time':
        force_fill = waited_days >= int(getattr(g, 'entry_search_trade_days', 20))
    ...
    if not trigger_hit and not force_fill:
        if g.debug:
            log.info('pending_entry_wait mode=%s current_date=%s waited_days=%s signal=%s threshold=%s' % (
                str(mode),
    ...
    if g.debug:
        log.info('pending_entry_completed mode=%s date=%s waited_days=%s signal=%s reason=%s' % (
            str(mode),
            str(to_date(context.current_dt)),
            str(waited_days),
            str(safe_round(current_signal, 6)),
...
def execute_target_weights(context, long_list, target_weight):
    execute_target_ratio(context, long_list, float(target_weight) * float(len(long_list)))


def get_entry_trigger_threshold(plan_id):
    plan_map = g.entry_trigger_library.get(plan_id, {})
    signal_map = plan_map.get(g.entry_signal_name, {})
    threshold_value = signal_map.get(g.entry_threshold_label, None)
...
def build_entry_feature_snapshot(factor_date, stocks):
    trade_days = get_trade_days(end_date=factor_date, count=int(getattr(g, 'entry_price_history_count', 25)))
    if trade_days is None or len(trade_days) < int(getattr(g, 'entry_price_history_count', 25)):
        return pd.DataFrame()
