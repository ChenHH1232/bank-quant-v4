...
def initialize(context):
    ...
    g.group_count = 5
    g.hold_bucket = 5
    # base_core_6 candidate with monthly relative-momentum-based discrete tilt.
    g.strategy_line = 'benchmark'  # 'primary' or 'benchmark'
    g.debug = True
    g.profitability_factor_names = ['indicator__roe', 'indicator__eps']
    g.quality_factor_names = [
    ...
    g.signal_hold_count = 8
    g.dynamic_tilt_rule_name = 'quality_70_if_rel_3m_le_0_else_50'
    g.rel_momentum_lookback_months = 3
    g.rel_momentum_negative_threshold = 0.0
    g.current_profitability_group_weight = 0.50
    g.current_quality_group_weight = 0.50
    g.current_tilt_state = '50_50'
    g.current_tilt_reason = 'equal_default'
    ...
    g.plan_usage_log = []
    g.previous_sleeve_state = None
    g.sleeve_nav_history = []
    g.tilt_signal_log = []
...
def do_rebalance(context):
    ...
    update_sleeve_nav_history(context, factor_date)
    tilt_info = resolve_dynamic_group_weights(factor_date)
    g.current_profitability_group_weight = float(tilt_info['profitability_weight'])
    g.current_quality_group_weight = float(tilt_info['quality_weight'])
    g.current_tilt_state = tilt_info['tilt_state']
    g.current_tilt_reason = tilt_info['reason']
    ...
    profitability_long_list = select_top_codes_by_score(score_df, 'profitability_score', g.signal_hold_count)
    quality_long_list = select_top_codes_by_score(score_df, 'quality_score', g.signal_hold_count)
    ...
    if g.debug:
        ...
        log.info(
            'tilt_state=%s reason=%s profit_weight=%.2f quality_weight=%.2f rel_momentum_3m=%s' % (
                g.current_tilt_state,
                g.current_tilt_reason,
                float(g.current_profitability_group_weight),
                float(g.current_quality_group_weight),
                format_pct(tilt_info.get('rel_momentum')),
            )
        )
    ...
    register_current_sleeve_state(
        factor_date=factor_date,
        profitability_codes=profitability_long_list,
        quality_codes=quality_long_list,
    )
    ...
    g.plan_usage_log.append({
        ...
        'tilt_state': g.current_tilt_state,
        'tilt_reason': g.current_tilt_reason,
        'profitability_group_weight': round(float(g.current_profitability_group_weight), 4),
        'quality_group_weight': round(float(g.current_quality_group_weight), 4),
        'rel_momentum_3m': safe_round(tilt_info.get('rel_momentum')),
...
def build_score_frame(context, stocks, factor_date, factor_specs):
    ...
    profitability_weight_sum = 0.0
    quality_weight_sum = 0.0
    profitability_weighted_score = None
    ...
    for spec in factor_specs:
        ...
        if factor_name in g.profitability_factor_names:
            profitability_weighted_score = (
                weighted_part
                if profitability_weighted_score is None
                else (profitability_weighted_score + weighted_part)
            )
            profitability_weight_sum += weight
        elif factor_name in g.quality_factor_names:
            quality_weighted_score = (
                weighted_part
                if quality_weighted_score is None
                else (quality_weighted_score + weighted_part)
            )
    ...
    if (
        profitability_weighted_score is None
        and quality_weighted_score is None
    ):
        if g.debug:
            log.info('group scores are empty on %s' % str(factor_date))
        return pd.DataFrame()

    profitability_score = (
        profitability_weighted_score / profitability_weight_sum
        if profitability_weighted_score is not None and profitability_weight_sum > 0
        else pd.Series(index=score_df.index, data=np.nan)
    )
    quality_score = (
        quality_weighted_score / quality_weight_sum
        if quality_weighted_score is not None and quality_weight_sum > 0
        else pd.Series(index=score_df.index, data=np.nan)
    )
    score_df['profitability_score'] = profitability_score
    score_df['quality_score'] = quality_score
    score_df['final_score'] = (
        score_df['profitability_score'] * float(g.current_profitability_group_weight)
        + score_df['quality_score'] * float(g.current_quality_group_weight)
    )
    score_df['factor_count'] = factor_count
    if g.debug:
        log.info(
            'group weight summary on %s: profit_factor_weight_sum=%.6f quality_factor_weight_sum=%.6f '
            'profit_group_weight=%.2f quality_group_weight=%.2f' % (
                str(factor_date),
                profitability_weight_sum,
                quality_weight_sum,
                float(g.current_profitability_group_weight),
                float(g.current_quality_group_weight),
            )
...
def select_top_codes_by_score(score_df, score_col, hold_count):
    if score_df is None or score_df.empty or score_col not in score_df.columns:
        return []
    subset = score_df[['code', score_col]].copy()
    subset[score_col] = pd.to_numeric(subset[score_col], errors='coerce')
    subset = subset.dropna(subset=[score_col]).sort_values([score_col, 'code'], ascending=[False, True])
    return subset.head(int(hold_count))['code'].astype(str).tolist()


def register_current_sleeve_state(factor_date, profitability_codes, quality_codes):
    state_factor_date = to_date(factor_date)
    if not g.sleeve_nav_history:
        g.sleeve_nav_history.append({
            'date': state_factor_date,
            'profitability_nav': 1.0,
            'quality_nav': 1.0,
            'relative_ratio': 1.0,
            'profitability_return': 0.0,
            'quality_return': 0.0,
        })
    g.previous_sleeve_state = {
        'factor_date': state_factor_date,
        'profitability_codes': list(profitability_codes),
        'quality_codes': list(quality_codes),
    }


def update_sleeve_nav_history(context, factor_date):
    if g.previous_sleeve_state is None:
        return

    current_factor_date = to_date(factor_date)
    previous_factor_date = g.previous_sleeve_state.get('factor_date')
    if previous_factor_date is None or previous_factor_date >= current_factor_date:
        return

    profitability_return = compute_equal_weight_basket_return(
        g.previous_sleeve_state.get('profitability_codes', []),
        previous_factor_date,
        current_factor_date,
    )
    quality_return = compute_equal_weight_basket_return(
        g.previous_sleeve_state.get('quality_codes', []),
        previous_factor_date,
        current_factor_date,
    )

    last_record = g.sleeve_nav_history[-1] if g.sleeve_nav_history else {
        'profitability_nav': 1.0,
        'quality_nav': 1.0,
    }
    profitability_nav = float(last_record.get('profitability_nav', 1.0)) * (1.0 + float(profitability_return))
    quality_nav = float(last_record.get('quality_nav', 1.0)) * (1.0 + float(quality_return))
    relative_ratio = safe_divide_scalar(profitability_nav, quality_nav)

    g.sleeve_nav_history.append({
        'date': current_factor_date,
        'profitability_nav': profitability_nav,
        'quality_nav': quality_nav,
        'relative_ratio': relative_ratio,
        'profitability_return': profitability_return,
        'quality_return': quality_return,
    })

    if g.debug:
        log.info(
            'sleeve_nav_update date=%s profit_return=%s quality_return=%s profit_nav=%.6f quality_nav=%.6f ratio=%s' % (
                str(current_factor_date),
                format_pct(profitability_return),
                format_pct(quality_return),
                profitability_nav,
                quality_nav,
                format_float(relative_ratio),
            )
        )


def resolve_dynamic_group_weights(factor_date):
    rel_momentum = compute_relative_momentum_signal(
        factor_date=factor_date,
        lookback_months=int(g.rel_momentum_lookback_months),
    )
    if pd.notna(rel_momentum) and rel_momentum <= float(g.rel_momentum_negative_threshold):
        result = {
            'profitability_weight': 0.30,
            'quality_weight': 0.70,
            'tilt_state': '30_70',
            'reason': 'quality_70_rel_3m_le_0',
    ...
    g.tilt_signal_log.append({
        'date': str(to_date(factor_date)),
        'tilt_state': result['tilt_state'],
        'reason': result['reason'],
        'rel_momentum': safe_round(result['rel_momentum']),
    })
    return result


def compute_relative_momentum_signal(factor_date, lookback_months):
    if not g.sleeve_nav_history:
        return np.nan

    current_date = to_date(factor_date)
    current_record = get_latest_nav_record_on_or_before(current_date)
    if current_record is None:
        return np.nan

    lookback_anchor = shift_months(current_date, -int(lookback_months))
    previous_record = get_latest_nav_record_on_or_before(lookback_anchor)
    if previous_record is None:
        return np.nan

    current_ratio = safe_divide_scalar(
        current_record.get('profitability_nav'),
        current_record.get('quality_nav'),
    )
    previous_ratio = safe_divide_scalar(
        previous_record.get('profitability_nav'),
        previous_record.get('quality_nav'),
    )
    if current_ratio is None or previous_ratio is None or previous_ratio <= 0:
        return np.nan
    return float(current_ratio / previous_ratio - 1.0)


def get_latest_nav_record_on_or_before(target_date):
    target_value = to_date(target_date)
    eligible = [item for item in g.sleeve_nav_history if item.get('date') is not None and item.get('date') <= target_value]
    if not eligible:
        return None
    eligible = sorted(eligible, key=lambda item: item['date'])
    return eligible[-1]


def compute_equal_weight_basket_return(codes, start_date, end_date):
    codes = list(codes)
    if not codes:
        return 0.0
    ...
    for code in codes:
        ...
        if price_df is None or len(price_df) < 2:
            continue
        try:
            close_series = pd.to_numeric(price_df['close'], errors='coerce').dropna()
        except Exception:
            close_series = pd.to_numeric(price_df, errors='coerce').dropna()
...
def shift_months(date_value, offset_months):
    current = to_date(date_value)
    year = current.year
    month = current.month + int(offset_months)
    while month <= 0:
        year -= 1
        month += 12
    while month > 12:
        year += 1
        month -= 12
    day = min(current.day, month_last_day(year, month))
...
def safe_divide_scalar(numerator, denominator):
