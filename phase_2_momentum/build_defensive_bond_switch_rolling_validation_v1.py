...
BOND_PANEL_PATH = MOMENTUM_DIR / "bond_defensive_panel_v1.csv"
OUT_DETAIL_PATH = MOMENTUM_DIR / "defensive_bond_switch_rolling_validation_v1_detail.csv"
OUT_SUMMARY_PATH = MOMENTUM_DIR / "defensive_bond_switch_rolling_validation_v1.md"
...
STRATEGY_SPECS = [
    {"strategy_name": "fundamental_only", "mode": "fixed", "fundamental_budget": 1.0, "momentum_budget": 0.0, "bond_budget": 0.0},
    {"strategy_name": "blend_60_40", "mode": "fixed", "fundamental_budget": 0.6, "momentum_budget": 0.4, "bond_budget": 0.0},
    {"strategy_name": "state_gated_bank_only", "mode": "state_bank_only"},
    {"strategy_name": "state_gated_bond_switch", "mode": "state_bond_switch"},
]
...
def load_bond_panel() -> pd.DataFrame:
    df = pd.read_csv(BOND_PANEL_PATH, encoding="utf-8-sig")
    df["rebalance_date"] = pd.to_datetime(df["rebalance_date"])
    df["bond_forward_return"] = pd.to_numeric(df["bond_forward_return"], errors="coerce")
    return df[["rebalance_date", "bond_code", "bond_forward_return"]].copy()
...
def build_joint_panel() -> pd.DataFrame:
    ...
    bond_df = load_bond_panel()
    bond_df = bond_df[bond_df["rebalance_date"] < pd.Timestamp("2021-05-01")].copy()
    ...
    merged = merged.merge(bond_df, on="rebalance_date", how="inner")
    merged[FUNDAMENTAL_TARGET_COL] = pd.to_numeric(merged[FUNDAMENTAL_TARGET_COL], errors="coerce")
    merged = merged.dropna(subset=[FUNDAMENTAL_TARGET_COL, "observable_state_score_v1", "bond_forward_return"]).copy()
    return merged
...
def get_bank_only_budgets(state_bucket: str) -> tuple[float, float, float]:
...
def get_bond_switch_budgets(state_bucket: str) -> tuple[float, float, float]:
    if state_bucket == "strong_up":
        return 0.6, 0.4, 0.0
    if state_bucket == "weak_down":
        return 0.6, 0.0, 0.4
...
def evaluate_group(
    ...
    bond_forward_return = float(pd.to_numeric(work["bond_forward_return"], errors="coerce").iloc[0]) if len(work) > 0 else 0.0
    bond_weight = max(0.0, float(bond_budget))
    bond_return_contribution = bond_weight * bond_forward_return
    stock_invested_weight = float(work["final_weight"].sum())
    invested_weight = stock_invested_weight + bond_weight

    return {
        "portfolio_return": float(work["weighted_return"].sum()) + bond_return_contribution,
        "stock_return": float(work["weighted_return"].sum()),
        "bond_return_contribution": float(bond_return_contribution),
        "bond_forward_return": float(bond_forward_return),
...
def build_detail_rows(panel_df: pd.DataFrame, folds: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for fold in folds:
        ...
        for rebalance_date, group in validation_df.groupby("rebalance_date", sort=True):
            group = group.dropna(subset=[FUNDAMENTAL_COMBO, "score__mom12", FUNDAMENTAL_TARGET_COL, "observable_state_score_v1", "bond_forward_return"]).copy()
            if len(group) < 5:
                continue
            ...
            for spec in STRATEGY_SPECS:
                if spec["mode"] == "state_bank_only":
                    fundamental_budget, momentum_budget, bond_budget = get_bank_only_budgets(state_bucket)
                elif spec["mode"] == "state_bond_switch":
                    fundamental_budget, momentum_budget, bond_budget = get_bond_switch_budgets(state_bucket)
                else:
                    fundamental_budget = float(spec["fundamental_budget"])
                    momentum_budget = float(spec["momentum_budget"])
                    bond_budget = float(spec["bond_budget"])

                evaluated = evaluate_group(group, fundamental_budget, momentum_budget, bond_budget)
                rows.append(
                    {
                        ...
                        "bond_forward_return": round(float(evaluated["bond_forward_return"]), 10),
                        "bond_return_contribution": round(float(evaluated["bond_return_contribution"]), 10),
                        "portfolio_return": round(float(evaluated["portfolio_return"]), 10),
                        "stock_return": round(float(evaluated["stock_return"]), 10),
                        "invested_weight": round(float(evaluated["invested_weight"]), 6),
                        "cash_weight": round(float(evaluated["cash_weight"]), 6),
                        "bond_weight": round(float(evaluated["bond_weight"]), 6),
    ...
    if not df.empty:
        ...
        summary_df = (
            ...
            .agg(
                ...
                mean_stock_return=("stock_return", lambda s: pd.to_numeric(s, errors="coerce").mean()),
                mean_bond_contribution=("bond_return_contribution", lambda s: pd.to_numeric(s, errors="coerce").mean()),
                ...
                mean_bond_weight=("bond_weight", lambda s: pd.to_numeric(s, errors="coerce").mean()),
            )
            .reset_index()
        ...
        for _, row in summary_df.iterrows():
            lines.append(
                ...
                f"mean_stock_return=`{format_float(row['mean_stock_return'])}` | "
                f"mean_bond_contribution=`{format_float(row['mean_bond_contribution'])}` | "
                f"mean_bond_weight=`{format_float(row['mean_bond_weight'])}` | "
                f"mean_cash_weight=`{format_float(row['mean_cash_weight'])}`"
            )

        state_mix_df = df[df["strategy_name"] == "state_gated_bond_switch"].copy()
        if not state_mix_df.empty:
            lines.extend(["", "Bond-switch state mix summary:"])
            for state_bucket in ["strong_up", "neutral_flat", "weak_down"]:
                ...
                lines.append(
                    ...
                    f"mean_bond_budget=`{format_float(pd.to_numeric(subset['bond_budget'], errors='coerce').mean(), 2)}`"
                )

        lines.extend(["", "Fold summary:"])
        for fold_id in sorted(df["fold_id"].unique()):
            fold_df = df[df["fold_id"] == fold_id].copy()
            for strategy_name in ["fundamental_only", "blend_60_40", "state_gated_bank_only", "state_gated_bond_switch"]:
                ...
                mean_bond = pd.to_numeric(subset["bond_weight"], errors="coerce").mean()
                lines.append(
                    f"- `{fold_id}` `{strategy_name}` | validation=`{subset['validation_start'].iloc[0]}` to `{subset['validation_end'].iloc[0]}` | "
                    f"dates=`{len(subset)}` | cum_return=`{format_float(cum_return)}` | "
                    f"mean_cash=`{format_float(mean_cash)}` | mean_bond=`{format_float(mean_bond)}`"
                )
