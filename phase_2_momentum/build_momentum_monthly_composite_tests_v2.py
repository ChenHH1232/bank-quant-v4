...
MONTHLY_SAMPLE_PATH = SCRIPT_DIR / "momentum_monthly_rebalance_panel_v2.csv"
RESULTS_PATH = SCRIPT_DIR / "momentum_monthly_composite_test_results_v2.csv"
SUMMARY_PATH = SCRIPT_DIR / "momentum_monthly_composite_test_results_v2.md"
...
COMPOSITE_SPECS = [
    {
        "factor_name": "combo_631_balanced",
    ...
    {
        "factor_name": "combo_631_midheavy",
        "target_label": "y_month_total_return_close",
        "components": [
            ("mom_3_1", 0.1),
            ("mom_6_1", 0.6),
...
def load_panel() -> pd.DataFrame:
    df = pd.read_csv(MONTHLY_SAMPLE_PATH, encoding="utf-8-sig")
...
def preprocess_composite(
    ...
    for factor_name, weight in components:
        ...
        train_z = zscore_with_train_params(clipped_train, mean_value, std_value)
        valid_z = zscore_with_train_params(clipped_valid, mean_value, std_value)

        train_parts.append(train_z * float(weight))
        valid_parts.append(valid_z * float(weight))
        used_weights.append(float(weight))

    if not train_parts or sum(used_weights) <= 0:
        train_df["_factor_value"] = np.nan
        validation_df["_factor_value"] = np.nan
        return train_df, validation_df

    weight_sum = float(sum(used_weights))
    train_df["_factor_value"] = sum(train_parts) / weight_sum
    validation_df["_factor_value"] = sum(valid_parts) / weight_sum
    return train_df, validation_df
...
def compute_window_metrics(window_df: pd.DataFrame, target_label: str, direction: str | None = None) -> dict[str, object]:
    sample_df = window_df[["rebalance_date", target_label, "_factor_value"]].copy()
...
def build_results(panel_df: pd.DataFrame) -> list[dict[str, object]]:
    panel_df = panel_df[panel_df["rebalance_stock_pool_flag_v2"] == 1].copy()
    ...
    for spec in COMPOSITE_SPECS:
        ...
        train_df, validation_df = preprocess_composite(
            panel_df[train_mask],
            panel_df[validation_mask],
            spec["components"],
        )
        train_metrics_raw = compute_window_metrics(train_df, target_label)
        ...
        validation_metrics_raw = compute_window_metrics(validation_df, target_label, direction=direction)
        ...
        results.append(
            {
                ...
                "components": " + ".join(["%s*%s" % (w, f) for f, w in spec["components"]]),
...
def write_summary(result_rows: list[dict[str, object]], monthly_panel: pd.DataFrame) -> None:
    monthly_in_pool = monthly_panel[monthly_panel["rebalance_stock_pool_flag_v2"] == 1].copy()
    ...
    lines = [
        ...
        f"- In-pool monthly rows: `{len(monthly_in_pool)}`",
        f"- Monthly rebalance dates: `{monthly_panel['rebalance_date'].nunique()}`",
        f"- Tested composites: `{len(result_rows)}`",
    ...
    if ranked.empty:
        ...
        for _, row in ranked.iterrows():
            lines.append(
                f"- `{row['factor_name']}` | status=`{row['final_status']}` | direction=`{row['direction']}` | val_ic=`{row['validation_rank_ic_mean']}` | val_spread=`{row['validation_top_minus_bottom']}` | components=`{row['components']}`"
            )
...
def main() -> None:
    panel_df = load_panel()
    results = build_results(panel_df)
    write_csv(results, RESULTS_PATH)
    write_summary(results, panel_df)
    print(RESULTS_PATH)
    print(SUMMARY_PATH)
