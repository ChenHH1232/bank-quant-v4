...
PANEL_PATH = SCRIPT_DIR / "momentum_factor_panel_v1.csv"
ANNUAL_SAMPLE_PATH = SCRIPT_DIR / "momentum_annual_rebalance_panel_v2.csv"
RESULTS_PATH = SCRIPT_DIR / "momentum_single_factor_test_results_v2.csv"
SUMMARY_PATH = SCRIPT_DIR / "momentum_single_factor_test_results_v2.md"
...
FACTOR_SPECS = [
    {"factor_name": "mom_1m", "factor_family": "momentum", "target_label": "y_year_total_return_close"},
    {"factor_name": "mom_3m", "factor_family": "momentum", "target_label": "y_year_total_return_close"},
    {"factor_name": "mom_6m", "factor_family": "momentum", "target_label": "y_year_total_return_close"},
    {"factor_name": "mom_12m", "factor_family": "momentum", "target_label": "y_year_total_return_close"},
    {"factor_name": "mom_12_1", "factor_family": "momentum", "target_label": "y_year_total_return_close"},
    {"factor_name": "mom_6_1", "factor_family": "momentum", "target_label": "y_year_total_return_close"},
    {"factor_name": "liq_money_1m", "factor_family": "liquidity", "target_label": "y_year_total_return_close"},
    {"factor_name": "liq_turnover_1m", "factor_family": "liquidity", "target_label": "y_year_total_return_close"},
]

# Keep the 5-year training / 2-year validation split unchanged.
# V2 only relaxes pass/fail thresholds so annual long-window momentum factors
# are not automatically discarded by small-sample mechanics.
TRAIN_MIN_DATES = 4
...
def build_annual_rebalance_panel(panel_df: pd.DataFrame) -> pd.DataFrame:
    may_trade_days = (
        panel_df.assign(year=panel_df["date"].dt.year, month=panel_df["date"].dt.month)
        .loc[lambda x: x["month"] == 5, ["date", "year"]]
        .drop_duplicates()
        .sort_values("date")
    ...
    sampled = panel_df.merge(may_trade_days, left_on="date", right_on="rebalance_date", how="inner").copy()
    sampled = sampled.drop(columns=["year", "date"])
    sampled = sampled.sort_values(["code", "rebalance_date"]).reset_index(drop=True)
    ...
    sampled["y_year_total_return_close"] = sampled["next_rebalance_close"].div(sampled["close"]) - 1.0

    output_rows: list[dict[str, object]] = []
    daily_groups = {code: grp.sort_values("date").copy() for code, grp in panel_df.groupby("code")}
    for row in sampled.itertuples(index=False):
        ...
        payload = row._asdict()
        payload["y_year_avg_daily_return_close"] = avg_daily_return
    ...
    out["next_rebalance_date"] = pd.to_datetime(out["next_rebalance_date"])
    out = out.sort_values(["rebalance_date", "code"]).reset_index(drop=True)

    liquidity_flags: list[int] = []
    market_cap_flags: list[int] = []
    pool_flags: list[int] = []
    for _, group in out.groupby("rebalance_date", sort=False):
        ...
        liquidity_flags.extend(liq_flag.tolist())
        market_cap_flags.extend(mcap_flag.tolist())
        pool_flags.extend(pool_flag.tolist())

    out["liquidity_top_80_flag"] = liquidity_flags
    out["market_cap_top_80_flag"] = market_cap_flags
...
def build_results(panel_df: pd.DataFrame) -> list[dict[str, object]]:
    panel_df = panel_df[panel_df["rebalance_stock_pool_flag"] == 1].copy()
...
def write_summary(result_rows: list[dict[str, object]], annual_panel: pd.DataFrame) -> None:
    annual_in_pool = annual_panel[annual_panel["rebalance_stock_pool_flag"] == 1].copy()
    ...
    focus = ranked[ranked["factor_name"].isin(["mom_12m", "mom_12_1", "mom_6_1", "liq_money_1m"])].copy() if not ranked.empty else ranked
    ...
    lines = [
        ...
        "",
        f"- Annual sample rows: `{len(annual_panel)}`",
        f"- In-pool annual rows: `{len(annual_in_pool)}`",
        f"- Annual rebalance dates: `{annual_panel['rebalance_date'].nunique()}`",
    ...
    if focus.empty:
        ...
        for _, row in focus.iterrows():
            lines.append(
                f"- `{row['factor_name']}` | status=`{row['final_status']}` | direction=`{row['direction']}` | train_rows=`{row['train_rows']}` | val_rows=`{row['validation_rows']}` | val_ic=`{row['validation_rank_ic_mean']}` | val_spread=`{row['validation_top_minus_bottom']}`"
            )
    ...
    if ranked.empty:
        ...
        for _, row in ranked.iterrows():
            lines.append(
                f"- `{row['factor_name']}` | status=`{row['final_status']}` | direction=`{row['direction']}` | val_ic=`{row['validation_rank_ic_mean']}` | val_spread=`{row['validation_top_minus_bottom']}`"
            )
    ...
    lines.extend(
        [
            "",
            "Output:",
            f"- [{ANNUAL_SAMPLE_PATH.name}]({ANNUAL_SAMPLE_PATH})",
...
def main() -> None:
    panel_df = load_panel()
    annual_panel = build_annual_rebalance_panel(panel_df)
    annual_panel_to_save = annual_panel.copy()
    annual_panel_to_save["rebalance_date"] = annual_panel_to_save["rebalance_date"].dt.strftime("%Y-%m-%d")
    annual_panel_to_save["next_rebalance_date"] = annual_panel_to_save["next_rebalance_date"].dt.strftime("%Y-%m-%d")
    annual_panel_to_save.to_csv(ANNUAL_SAMPLE_PATH, index=False, encoding="utf-8-sig")

    results = build_results(annual_panel)
    write_csv(results, RESULTS_PATH)
    write_summary(results, annual_panel)
    print(ANNUAL_SAMPLE_PATH)
    print(RESULTS_PATH)
    print(SUMMARY_PATH)
