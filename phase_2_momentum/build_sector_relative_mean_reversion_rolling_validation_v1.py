...
RESULTS_PATH = SCRIPT_DIR / "sector_relative_mean_reversion_rolling_validation_v1_results.csv"
SUMMARY_PATH = SCRIPT_DIR / "sector_relative_mean_reversion_rolling_validation_v1.md"
...
FACTOR_SPECS = [
    {"factor_name": "rel_sector_rev_5d", "label": "5d excess drop vs sector proxy"},
    {"factor_name": "rel_sector_rev_10d", "label": "10d excess drop vs sector proxy"},
    {"factor_name": "rel_sector_rev_20d", "label": "20d excess drop vs sector proxy"},
]
...
def load_panel() -> pd.DataFrame:
    ...
    for lookback in [5, 10, 20]:
        source_col = f"rev_{lookback}d"
        proxy_col = f"sector_proxy_rev_{lookback}d"
        rel_col = f"rel_sector_rev_{lookback}d"
        df[proxy_col] = df.groupby("rebalance_date")[source_col].transform(
            lambda s: pd.to_numeric(s, errors="coerce").mean()
        )
        df[rel_col] = pd.to_numeric(df[source_col], errors="coerce") - pd.to_numeric(df[proxy_col], errors="coerce")
...
def write_summary(rows: list[dict[str, object]], fold_count: int) -> None:
    ...
    if not df.empty:
        ...
        for _, row in summary_df.iterrows():
            lines.append(
                ...
                f"mean_usable_dates=`{format_float(row['mean_usable_dates'], 2)}`"
            )
...
def main() -> None:
    panel_df = load_panel()
    folds = load_folds()
    rows = build_result_rows(panel_df, folds)
    write_csv(RESULTS_PATH, rows)
    write_summary(rows, len(folds))
    print(RESULTS_PATH)
    print(SUMMARY_PATH)
