...
PANEL_PATH = SCRIPT_DIR / "mean_reversion_monthly_rebalance_panel_v1.csv"
...
RESULTS_PATH = SCRIPT_DIR / "sector_relative_mean_reversion_rolling_validation_v2_results.csv"
SUMMARY_PATH = SCRIPT_DIR / "sector_relative_mean_reversion_rolling_validation_v2.md"
...
FACTOR_SPECS = [
    {"factor_name": "rel_etf_rev_5d", "label": "5d excess drop vs 512800"},
    {"factor_name": "rel_etf_rev_10d", "label": "10d excess drop vs 512800"},
    {"factor_name": "rel_etf_rev_20d", "label": "20d excess drop vs 512800"},
]
...
def load_panel() -> pd.DataFrame:
    ...
    for col in [READY_FLAG, TARGET_COL, "rev_5d", "rev_10d", "rev_20d"]:
...
def load_etf_history() -> pd.DataFrame:
    ...
    for lookback in [5, 10, 20]:
        df[f"etf_rev_{lookback}d"] = calc_return_series(df["close"], lookback)
    return df.sort_values("date").reset_index(drop=True)


def calc_return_series(close_series: pd.Series, lookback_days: int) -> pd.Series:
    base = pd.to_numeric(close_series, errors="coerce").shift(lookback_days)
    current = pd.to_numeric(close_series, errors="coerce")
    out = current / base - 1.0
    out[(base.abs() <= 1e-12) | base.isna() | current.isna()] = np.nan
    return out


def attach_etf_relative_features(panel_df: pd.DataFrame, etf_df: pd.DataFrame) -> pd.DataFrame:
    etf_cols = ["date", "etf_rev_5d", "etf_rev_10d", "etf_rev_20d"]
    merged = panel_df.merge(etf_df[etf_cols], left_on="rebalance_date", right_on="date", how="left")
    merged = merged.drop(columns=["date"])
    merged["rel_etf_rev_5d"] = pd.to_numeric(merged["rev_5d"], errors="coerce") - pd.to_numeric(merged["etf_rev_5d"], errors="coerce")
    merged["rel_etf_rev_10d"] = pd.to_numeric(merged["rev_10d"], errors="coerce") - pd.to_numeric(merged["etf_rev_10d"], errors="coerce")
    merged["rel_etf_rev_20d"] = pd.to_numeric(merged["rev_20d"], errors="coerce") - pd.to_numeric(merged["etf_rev_20d"], errors="coerce")
    ...
    if not df.empty:
        summary_df = (
            ...
            .agg(
                ...
                mean_usable_dates=("usable_dates", lambda s: pd.to_numeric(s, errors="coerce").mean()),
