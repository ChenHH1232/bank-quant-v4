...
RESULTS_PATH = SCRIPT_DIR / "sector_relative_mean_reversion_rolling_validation_v3_results.csv"
SUMMARY_PATH = SCRIPT_DIR / "sector_relative_mean_reversion_rolling_validation_v3.md"
...
FACTOR_SPECS = [
    {"factor_name": "ratio_to_ma_5d", "label": "price ratio vs 5d mean"},
    {"factor_name": "ratio_to_ma_10d", "label": "price ratio vs 10d mean"},
    {"factor_name": "ratio_to_ma_20d", "label": "price ratio vs 20d mean"},
]
...
def build_ratio_feature_panel(daily_df: pd.DataFrame, etf_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in merged.groupby("code", sort=False):
        ...
        for lookback in [5, 10, 20]:
            ratio_ma = one["ratio_close"].rolling(lookback, min_periods=lookback).mean()
            factor_col = f"ratio_to_ma_{lookback}d"
            one[factor_col] = one["ratio_close"] / ratio_ma - 1.0
            one.loc[(ratio_ma.abs() <= 1e-12) | ratio_ma.isna(), factor_col] = np.nan
        result_parts.append(one[["date", "code", "ratio_to_ma_5d", "ratio_to_ma_10d", "ratio_to_ma_20d"]])
