...
PRICE_STATE_PATH = MOMENTUM_DIR / "momentum_forward_state_score_v1.csv"
DETERIORATION_STATE_PATH = MOMENTUM_DIR / "fundamental_deterioration_date_panel_v1.csv"
OUT_DETAIL_PATH = MOMENTUM_DIR / "state_proxy_comparison_rolling_validation_v1_detail.csv"
OUT_SUMMARY_PATH = MOMENTUM_DIR / "state_proxy_comparison_rolling_validation_v1.md"
...
STRATEGY_SPECS = [
    ...
    {"strategy_name": "price_state_proxy", "mode": "price_state"},
    {"strategy_name": "deterioration_state_proxy", "mode": "deterioration_state"},
    {"strategy_name": "combined_state_proxy", "mode": "combined_state"},
]
...
def load_price_state_proxy() -> pd.DataFrame:
    df = pd.read_csv(PRICE_STATE_PATH, encoding="utf-8-sig")
    df["rebalance_date"] = pd.to_datetime(df["rebalance_date"])
    df["price_state_score"] = (
    ...
    return df[["rebalance_date", "price_state_score"]].copy()


def load_deterioration_state_proxy() -> pd.DataFrame:
    df = pd.read_csv(DETERIORATION_STATE_PATH, encoding="utf-8-sig")
    df["rebalance_date"] = pd.to_datetime(df["rebalance_date"])
...
def preprocess_state_series(full_df: pd.DataFrame, train_mask: pd.Series, col_name: str) -> pd.Series:
    values = pd.to_numeric(full_df[col_name], errors="coerce")
    train_values = values[train_mask].dropna()
    if train_values.empty:
        return pd.Series(index=full_df.index, dtype="float64")
    lower = float(train_values.quantile(0.01))
...
def build_joint_panel() -> pd.DataFrame:
    ...
    price_state_df = load_price_state_proxy()
    price_state_df = price_state_df[price_state_df["rebalance_date"] < pd.Timestamp("2021-05-01")].copy()
    deterioration_df = load_deterioration_state_proxy()
    deterioration_df = deterioration_df[deterioration_df["rebalance_date"] < pd.Timestamp("2021-05-01")].copy()

    merged = fundamental_df.merge(momentum_df, on=["rebalance_date", "code"], how="inner")
    merged = merged.merge(price_state_df, on="rebalance_date", how="inner")
    merged = merged.merge(deterioration_df, on="rebalance_date", how="inner")
    merged[FUNDAMENTAL_TARGET_COL] = pd.to_numeric(merged[FUNDAMENTAL_TARGET_COL], errors="coerce")
    merged = merged.dropna(
        subset=[
            FUNDAMENTAL_TARGET_COL,
            "price_state_score",
            "deterioration_state_raw",
        ]
    ).copy()
...
def build_detail_rows(panel_df: pd.DataFrame, folds: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for fold in folds:
        ...
        scored_df["z__price_state"] = preprocess_state_series(scored_df, train_mask, "price_state_score")
        scored_df["z__deterioration_state"] = preprocess_state_series(scored_df, train_mask, "deterioration_state_raw")
        scored_df["combined_state_score"] = scored_df["z__price_state"] + scored_df["z__deterioration_state"]
        ...
        cut_map = {}
        for proxy_col in ["price_state_score", "deterioration_state_raw", "combined_state_score"]:
            cut_map[proxy_col] = (
                float(train_state_df[proxy_col].quantile(0.33)),
                float(train_state_df[proxy_col].quantile(0.67)),
            )
        ...
        for rebalance_date, group in validation_df.groupby("rebalance_date", sort=True):
            ...
            price_state_value = float(group["price_state_score"].iloc[0])
            deterioration_state_value = float(group["deterioration_state_raw"].iloc[0])
            combined_state_value = float(group["combined_state_score"].iloc[0])

            state_bucket_map = {
                "price_state_proxy": classify_state(
                    price_state_value,
                    cut_map["price_state_score"][0],
                    cut_map["price_state_score"][1],
                ),
                "deterioration_state_proxy": classify_state(
                    deterioration_state_value,
                    cut_map["deterioration_state_raw"][0],
                    cut_map["deterioration_state_raw"][1],
                ),
                "combined_state_proxy": classify_state(
                    combined_state_value,
                    cut_map["combined_state_score"][0],
                    cut_map["combined_state_score"][1],
                ),
            }
            ...
            for spec in STRATEGY_SPECS:
                if spec["mode"] == "fixed":
                    ...
                    state_bucket = state_bucket_map[spec["strategy_name"]]
                    fundamental_budget, momentum_budget = get_state_budgets(state_bucket)
                ...
                rows.append(
                    {
                        ...
                        "price_state_score": round(price_state_value, 6),
                        "price_state_low_cut": round(cut_map["price_state_score"][0], 6),
                        "price_state_high_cut": round(cut_map["price_state_score"][1], 6),
                        "deterioration_state_score": round(deterioration_state_value, 6),
                        "deterioration_state_low_cut": round(cut_map["deterioration_state_raw"][0], 6),
                        "deterioration_state_high_cut": round(cut_map["deterioration_state_raw"][1], 6),
                        "combined_state_score": round(combined_state_value, 6),
                        "combined_state_low_cut": round(cut_map["combined_state_score"][0], 6),
                        "combined_state_high_cut": round(cut_map["combined_state_score"][1], 6),
...
def write_summary(rows: list[dict[str, object]], folds: list[dict[str, object]]) -> None:
    ...
    if not df.empty:
        ...
        lines.extend(["", "Proxy state mix summary:"])
        for strategy_name in ["price_state_proxy", "deterioration_state_proxy", "combined_state_proxy"]:
            subset_df = df[df["strategy_name"] == strategy_name].copy()
            if subset_df.empty:
                continue
            ...
            for state_bucket in ["strong_up", "neutral_flat", "weak_down"]:
                ...
                if subset.empty:
                    continue
                lines.append(
                    f"- `{strategy_name}` `{state_bucket}` | snapshots=`{len(subset)}` | "
