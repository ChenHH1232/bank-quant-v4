...
def initialize(context):
    # Switch here when comparing the two frozen executable variants.
    ...
    g.monthly_release_threshold = 0.75  # Frozen pre-2021 train-set q50 of bank mom_6_1 positive ratio.
    ...
    # Slow fundamental sleeve cache used only by the active candidate.
    g.slow_fundamental_holdings = {}
    ...
    g.candidate_annual_state_schedule = [
        {"effective_date": "2021-05-31", "state_bucket": "neutral_flat", "source_note": "negative deterioration_ratio q33/q67 from 2021-05-06"},
        {"effective_date": "2022-05-05", "state_bucket": "strong_up", "source_note": "negative deterioration_ratio q33/q67 from 2022-05-05"},
        {"effective_date": "2023-05-04", "state_bucket": "neutral_flat", "source_note": "negative deterioration_ratio q33/q67 from 2023-05-04"},
        {"effective_date": "2024-05-06", "state_bucket": "strong_up", "source_note": "negative deterioration_ratio q33/q67 from 2024-05-06"},
        {"effective_date": "2025-05-06", "state_bucket": "strong_up", "source_note": "negative deterioration_ratio q33/q67 from 2025-05-06"},
        {"effective_date": "2026-05-06", "state_bucket": "strong_up", "source_note": "carry forward last frozen annual breadth state"},
    ]
...
def maybe_rebalance(context):
    ...
    if len(context.portfolio.positions) == 0:
        ...
        if due_plan is not None:
            ...
            should_rebalance = True
            rebalance_reason = "monthly_refresh"
...
def do_rebalance(context, rebalance_reason):
    ...
    is_fundamental_date = bool(get_due_unexecuted_fundamental_plan(current_date) is not None) or rebalance_reason == "initial_build"
    is_monthly_check_date = is_first_trade_day_of_month(current_date)
    ...
    budget_context = build_budget_context(current_date, factor_date, is_monthly_check_date)
    if budget_context is None:
        log.info("No budget context on %s variant=%s" % (str(current_date), g.strategy_variant))
        return False

    fundamental_budget = float(budget_context["fundamental_budget"])
    momentum_budget = float(budget_context["momentum_budget"])

    momentum_context = build_momentum_context(factor_date)
    if momentum_context is None:
        log.info("Momentum context is unavailable on %s" % str(factor_date))
        return False

    momentum_df = momentum_context["score_df"]
    if momentum_budget > 1e-12 and momentum_df.empty:
        log.info("Momentum score frame is empty on %s" % str(factor_date))
        return False

    if g.strategy_variant == "fixed_60_40":
        ...
        fundamental_trade_permission = "unconstrained_monthly_rebuild"
        actual_fundamental_weight = sum_weight_map(fundamental_weights)
        actual_momentum_weight = sum_weight_map(momentum_weights)
    else:
        fundamental_target = build_fundamental_engine_weights(active_plan["codes"], fundamental_budget)
        fundamental_weights, fundamental_trade_permission = build_slow_fundamental_weights(
            active_plan["codes"],
            fundamental_target,
            fundamental_budget,
            is_fundamental_date,
    ...
    if g.debug:
        log.info("========== V4 active-mainline rebalance ==========")
        log.info(
        ...
        log.info("budget_context=%s" % str(budget_context))
        log.info(
            "execution_flags=%s"
            % str(
                {
                    "is_fundamental_date": is_fundamental_date,
                    "is_monthly_check_date": is_monthly_check_date,
                    "fundamental_trade_permission": fundamental_trade_permission,
        ...
        log.info(
            "release_signal_summary=%s"
            % str(
                {
                    "release_signal_name": "mom6_positive_ratio",
                    "release_signal_value": safe_round(momentum_context["release_signal_value"], 6),
                    "release_signal_threshold": safe_round(g.monthly_release_threshold, 6),
                    "release_signal_universe_count": int(momentum_context["release_universe_count"]),
                }
            )
        )
    ...
    g.latest_rebalance_log = {
        "mode": "active_mainline_execution_v1",
        ...
        "annual_anchor_state": budget_context["annual_anchor_state"],
        "effective_state_bucket": budget_context["effective_state_bucket"],
        "monthly_release_checked": budget_context["monthly_release_checked"],
        "monthly_release_triggered": budget_context["monthly_release_triggered"],
        "release_signal_value": safe_round(momentum_context["release_signal_value"], 6),
        "release_signal_threshold": safe_round(g.monthly_release_threshold, 6),
        "fundamental_trade_permission": fundamental_trade_permission,
        "target_fundamental_budget": safe_round(fundamental_budget, 6),
        "target_momentum_budget": safe_round(momentum_budget, 6),
        "actual_fundamental_weight": safe_round(actual_fundamental_weight, 6),
        "actual_momentum_weight": safe_round(actual_momentum_weight, 6),
...
def get_budgets_for_state(state_bucket):
    if state_bucket == "strong_up" or state_bucket == "fixed":
        return 0.60, 0.40
...
def build_budget_context(current_date, factor_date, is_monthly_check_date):
    if g.strategy_variant == "fixed_60_40":
        fundamental_budget, momentum_budget = get_budgets_for_state("fixed")
        return {
            "annual_anchor_state": "fixed",
            "effective_state_bucket": "fixed",
            "monthly_release_checked": bool(is_monthly_check_date),
            "monthly_release_triggered": False,
            "fundamental_budget": fundamental_budget,
            "momentum_budget": momentum_budget,
            "source_note": "fixed dual-engine baseline",
        }

    annual_plan = get_active_candidate_annual_state_plan(current_date)
    if annual_plan is None:
        return None

    annual_anchor_state = str(annual_plan["state_bucket"])
    effective_state_bucket = annual_anchor_state
    monthly_release_triggered = False

    if annual_anchor_state == "weak_down" and is_monthly_check_date:
        release_value = compute_monthly_release_value(factor_date)
        if pd.notna(release_value) and float(release_value) >= g.monthly_release_threshold:
            effective_state_bucket = "neutral_flat"
            monthly_release_triggered = True

    fundamental_budget, momentum_budget = get_budgets_for_state(effective_state_bucket)
    return {
        "annual_anchor_state": annual_anchor_state,
        "effective_state_bucket": effective_state_bucket,
        "monthly_release_checked": bool(is_monthly_check_date),
        "monthly_release_triggered": monthly_release_triggered,
        "fundamental_budget": fundamental_budget,
        "momentum_budget": momentum_budget,
        "source_note": annual_plan["source_note"],
    }
...
def get_active_candidate_annual_state_plan(current_date):
    active = None
    for plan in g.candidate_annual_state_schedule:
        plan_date = pd.Timestamp(plan["effective_date"]).date()
        if plan_date <= current_date:
            active = plan
    return active
...
def build_slow_fundamental_weights(active_codes, target_weight_map, target_budget, is_fundamental_date):
    if is_fundamental_date:
        g.slow_fundamental_holdings = dict(target_weight_map)
        return dict(g.slow_fundamental_holdings), "rebuild_allowed"

    current_map = {}
    tradable_active_codes = set(get_tradable_stocks(active_codes))
    for code, value in g.slow_fundamental_holdings.items():
        if code in tradable_active_codes and float(value) > 1e-12:
            current_map[code] = float(value)

    current_sum = sum_weight_map(current_map)
    if current_sum > target_budget + 1e-12:
        scale = target_budget / current_sum if current_sum > 1e-12 else 0.0
        current_map = {
            code: float(weight) * scale
            for code, weight in current_map.items()
            if float(weight) * scale > 1e-12
        }

    g.slow_fundamental_holdings = dict(current_map)
    return dict(current_map), "sell_only_no_rebuild"


def build_momentum_context(factor_date):
    ...
    release_df = feature_df[feature_df["pool_flag"] == 1].copy()
    if release_df.empty:
        release_df = feature_df[feature_df["price_ready"] == 1].copy()
    release_signal_value = np.nan
    release_universe_count = 0
    if not release_df.empty:
        mom6 = pd.to_numeric(release_df["mom_6_1"], errors="coerce").dropna()
        release_universe_count = int(len(mom6))
        if not mom6.empty:
            release_signal_value = float((mom6 > 0).mean())

    score_df = release_df.copy()
    if not score_df.empty:
        score_df["final_score"] = score_series(score_df["mom_12_1"], "larger_better")
        score_df = score_df.dropna(subset=["final_score"]).copy()
        score_df = score_df.sort_values(["final_score", "code"], ascending=[False, True]).reset_index(drop=True)

    return {
        "feature_df": feature_df,
        "score_df": score_df,
        "release_signal_value": release_signal_value,
        "release_universe_count": release_universe_count,
    }


def compute_monthly_release_value(factor_date):
    context = build_momentum_context(factor_date)
    if context is None:
        return np.nan
    return context["release_signal_value"]
...
def summarize_weight_map(weight_map):
    ...
    return {
        ...
        "sum": safe_round(sum_weight_map(weight_map), 6),
        "max": safe_round(max(values), 6),
        "top": sorted(weight_map.items(), key=lambda item: (-item[1], item[0]))[:10],
    }


def sum_weight_map(weight_map):
    if weight_map is None:
        return 0.0
    total = 0.0
    for _, value in weight_map.items():
        try:
            total += float(value)
        except Exception:
            continue
...
def build_momentum_preview(df):
    ...
    for _, row in df.iterrows():
        preview.append(
            "%s:score=%s,m6=%s,m12=%s"
            % (
                row["code"],
                str(safe_round(row.get("final_score"), 4)),
                str(safe_round(row.get("mom_6_1"), 4)),
