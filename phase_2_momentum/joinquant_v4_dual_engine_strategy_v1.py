...
def initialize(context):
    ...
    # Dual-engine allocation from the first-pass rolling test:
    # - fundamental engine uses the approved annual/quasi-quarterly pool
    # - momentum engine uses monthly 12-1 ranking
    # - final portfolio only combines weights; it does not mix raw factor scores
    g.fundamental_budget = 0.60
    g.momentum_budget = 0.40
...
def do_rebalance(context, rebalance_reason):
    ...
    fundamental_weights = build_fundamental_engine_weights(active_plan["codes"])
    momentum_df = build_momentum_score_frame(factor_date)
    if momentum_df.empty:
        log.info("Momentum score frame is empty on %s" % str(factor_date))
        return False
    momentum_weights = build_momentum_engine_weights(momentum_df)
    ...
    if g.debug:
        log.info("========== V4 dual-engine rebalance ==========")
        log.info(
            "current_date=%s factor_date=%s reason=%s fundamental_plan=%s"
            % (str(current_date), str(factor_date), rebalance_reason, active_plan["effective_date"])
        )
        log.info(
            "budget_summary=%s"
            % str(
...
def build_fundamental_engine_weights(codes):
    ...
    return build_ranked_weight_map(score_df, "rank_score", g.fundamental_budget, g.engine_stock_cap)
...
def build_momentum_engine_weights(momentum_df):
    return build_ranked_weight_map(momentum_df, "final_score", g.momentum_budget, g.engine_stock_cap)
