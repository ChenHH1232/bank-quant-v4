...
ROLLING_RESULTS_PATH = SCRIPT_DIR / "momentum_rolling_validation_v1_results.csv"

OUTPUT_DETAIL_PATH = SCRIPT_DIR / "momentum_annual_strategy_backtest_v1_detail.csv"
OUTPUT_SUMMARY_PATH = SCRIPT_DIR / "momentum_annual_strategy_backtest_v1.md"
...
def build_detail_rows(panel_df: pd.DataFrame, plan_df: pd.DataFrame) -> list[dict[str, object]]:
    ...
    for plan in plan_df.itertuples(index=False):
        review_date = pd.Timestamp(plan.review_start)
        factor_name = str(plan.selected_alpha_factor)
        direction = str(plan.selected_alpha_direction)
        ...
        group["y_year_total_return_close"] = pd.to_numeric(group["y_year_total_return_close"], errors="coerce")
        group = group.dropna(subset=["_score", "y_year_total_return_close"])
        ...
        top_return = float(top_group["y_year_total_return_close"].mean())
        bottom_return = float(bottom_group["y_year_total_return_close"].mean())
        ...
        rows.append(
            {
                ...
                "rebalance_date": review_date.strftime("%Y-%m-%d"),
                "next_rebalance_date": pd.Timestamp(top_group["next_rebalance_date"].iloc[0]).strftime("%Y-%m-%d")
                if pd.notna(top_group["next_rebalance_date"].iloc[0])
                else "",
...
def write_summary(rows: list[dict[str, object]], plan_df: pd.DataFrame) -> None:
    if not rows:
        OUTPUT_SUMMARY_PATH.write_text("# Momentum Annual Strategy Backtest V1\n\n- no rows", encoding="utf-8")
        return
    ...
    lines = [
        ...
        "",
        f"- review rebalance rows: `{len(df)}`",
        f"- candidate review folds in rolling plan: `{len(plan_df[plan_df['scenario_name'] == 'alpha_only'])}`",
    ...
    for row in df.sort_values("rebalance_date").itertuples(index=False):
        lines.append(
            f"- `{row.rebalance_date.strftime('%Y-%m-%d')}` | alpha=`{row.selected_alpha_factor}` | "
    ...
    missing_review = plan_df[
        (plan_df["scenario_name"] == "alpha_only")
        & (~plan_df["review_start"].dt.strftime("%Y-%m-%d").isin(df["rebalance_date"].dt.strftime("%Y-%m-%d")))
    ].copy()
    if not missing_review.empty:
        lines.extend(["", "Unscored review folds:"])
        for row in missing_review.itertuples(index=False):
            lines.append(
                f"- `{row.fold_id}` | review_start=`{row.review_start.strftime('%Y-%m-%d')}` | reason=`missing next annual return label in current panel`"
            )
