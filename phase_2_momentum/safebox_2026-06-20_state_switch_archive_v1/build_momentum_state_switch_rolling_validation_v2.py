...
OUT_FOLDS_PATH = SCRIPT_DIR / "momentum_state_switch_rolling_validation_v2_folds.csv"
OUT_RESULTS_PATH = SCRIPT_DIR / "momentum_state_switch_rolling_validation_v2_results.csv"
OUT_SUMMARY_PATH = SCRIPT_DIR / "momentum_state_switch_rolling_validation_v2.md"
...
def load_state_scores() -> pd.DataFrame:
    ...
    return df[["rebalance_date", "state_score_v1"]]
...
def build_folds(dates: list[pd.Timestamp]) -> list[dict[str, object]]:
    ...
    for start_idx in range(0, len(dates) - span + 1):
        ...
        folds.append(
            {
                "fold_id": f"mom_sswitch_rt2_fold_{len(folds) + 1:03d}",
        ...
        rows.append(
            {
                ...
                "test_mid_state_months": int((test_df["state_bucket"] == "mid_use_mom12").sum()),
                "test_low_state_months": int((test_df["state_bucket"] == "low_use_mom12").sum()),
            }
        )
    return rows
...
def write_summary(result_rows: list[dict[str, object]]) -> None:
    ...
    if len(df) > 0:
        switch_returns = pd.to_numeric(df["test_switch_total_return"], errors="coerce")
        mom6_returns = pd.to_numeric(df["test_mom6_total_return"], errors="coerce")
        mom12_returns = pd.to_numeric(df["test_mom12_total_return"], errors="coerce")

        lines.extend(
            [
                "",
                "Average fold test results:",
                f"- mean_test_switch_total_return=`{format_float(switch_returns.mean())}`",
                f"- mean_test_mom6_total_return=`{format_float(mom6_returns.mean())}`",
                f"- mean_test_mom12_total_return=`{format_float(mom12_returns.mean())}`",
                f"- mean_test_switch_annualized=`{format_float(pd.to_numeric(df['test_switch_annualized'], errors='coerce').mean())}`",
                "",
                "Win counts:",
                f"- test switch > mom6 folds: `{int((switch_returns > mom6_returns).sum())}`",
                f"- test switch > mom12 folds: `{int((switch_returns > mom12_returns).sum())}`",
                f"- test mom12 > mom6 folds: `{int((mom12_returns > mom6_returns).sum())}`",
                "",
                "Average state usage in test windows:",
                f"- high-state months using `mom_6_1`: `{format_float(pd.to_numeric(df['test_high_state_months'], errors='coerce').mean(), 2)}`",
                f"- mid-state months using `mom_12_1`: `{format_float(pd.to_numeric(df['test_mid_state_months'], errors='coerce').mean(), 2)}`",
                f"- low-state months using `mom_12_1`: `{format_float(pd.to_numeric(df['test_low_state_months'], errors='coerce').mean(), 2)}`",
                "",
                "Interpretation:",
