...
OUT_RESULTS_PATH = SCRIPT_DIR / "momentum_state_switch_threshold_test_v1.csv"
OUT_SUMMARY_PATH = SCRIPT_DIR / "momentum_state_switch_threshold_test_v1.md"
...
THRESHOLD_SPECS = [
    {"rule_name": "top33_use_mom6", "high_quantile": 0.67},
    {"rule_name": "top25_use_mom6", "high_quantile": 0.75},
    {"rule_name": "top20_use_mom6", "high_quantile": 0.80},
    {"rule_name": "top15_use_mom6", "high_quantile": 0.85},
]
...
def load_state_scores() -> pd.DataFrame:
    df = pd.read_csv(STATE_V2_PATH, encoding="utf-8-sig")
    df["rebalance_date"] = pd.to_datetime(df["rebalance_date"])
    return df[df["rebalance_date"] < pd.Timestamp("2021-01-01")][["rebalance_date", "state_score_v2"]].copy()
...
def build_folds(dates: list[pd.Timestamp]) -> list[dict[str, object]]:
    ...
    for start_idx in range(0, len(dates) - span + 1):
        ...
        folds.append(
            {
                "fold_id": f"mom_sswitch_th_fold_{len(folds) + 1:03d}",
...
def build_switch_frame(return_df: pd.DataFrame, score_df: pd.DataFrame, threshold_value: float) -> pd.DataFrame:
    out = score_df.merge(return_df, on="rebalance_date", how="left").copy()
    out["use_mom6"] = out["state_score_v2"] >= threshold_value
    out["selected_return"] = np.where(out["use_mom6"], out["mom_6_1_portfolio_return"], out["mom_12_1_portfolio_return"])
    return out
...
def build_result_rows(full_df: pd.DataFrame, folds: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for fold in folds:
        ...
        train_scores = pd.to_numeric(full_df.loc[train_mask, "state_score_v2"], errors="coerce").dropna()
        if len(train_scores) == 0:
            continue

        base_return_df = full_df[["rebalance_date", "mom_6_1_portfolio_return", "mom_12_1_portfolio_return"]].copy()
        score_df = full_df[["rebalance_date", "state_score_v2"]].copy()
        ...
        for spec in THRESHOLD_SPECS:
            threshold_value = float(train_scores.quantile(spec["high_quantile"]))
            switched = build_switch_frame(base_return_df, score_df, threshold_value=threshold_value)
            test_df = switched[test_mask].copy()
            test_switch = summarize_period(test_df, "selected_return")
            rows.append(
                {
                    ...
                    "train_threshold": round(threshold_value, 6),
                    "test_switch_total_return": round(float(test_switch["total_return"]), 8) if not pd.isna(test_switch["total_return"]) else "",
                    "test_switch_annualized": round(float(test_switch["annualized_return"]), 8) if not pd.isna(test_switch["annualized_return"]) else "",
                    ...
                    "test_use_mom6_months": int(test_df["use_mom6"].sum()),
                }
            )
    return rows
...
def write_summary(result_rows: list[dict[str, object]]) -> None:
    ...
    if not df.empty:
        ...
        summary_df = (
            ...
            .agg(
                folds=("fold_id", "count"),
                mean_test_return=("test_switch_total_return", lambda s: pd.to_numeric(s, errors="coerce").mean()),
                mean_test_annualized=("test_switch_annualized", lambda s: pd.to_numeric(s, errors="coerce").mean()),
                win_vs_mom6=("test_switch_total_return", lambda s: int((pd.to_numeric(s, errors="coerce") > pd.to_numeric(df.loc[s.index, "test_mom6_total_return"], errors="coerce")).sum())),
                win_vs_mom12=("test_switch_total_return", lambda s: int((pd.to_numeric(s, errors="coerce") > pd.to_numeric(df.loc[s.index, "test_mom12_total_return"], errors="coerce")).sum())),
                avg_use_mom6_months=("test_use_mom6_months", lambda s: pd.to_numeric(s, errors="coerce").mean()),
            )
            .reset_index()
            .sort_values(["mean_test_return", "win_vs_mom12"], ascending=[False, False])
        )
        for _, row in summary_df.iterrows():
            lines.append(
                f"- `{row['rule_name']}` | folds=`{int(row['folds'])}` | mean_test_return=`{format_float(row['mean_test_return'])}` | win_vs_mom6=`{int(row['win_vs_mom6'])}` | win_vs_mom12=`{int(row['win_vs_mom12'])}` | avg_use_mom6_months=`{format_float(row['avg_use_mom6_months'], 2)}`"
            )
        ...
        lines.extend(
            [
                ...
                f"- `{best['rule_name']}` with mean test return `{format_float(best['mean_test_return'])}`",
            ]
        )
    lines.extend(["", "Output:", f"- [{OUT_RESULTS_PATH.name}]({OUT_RESULTS_PATH})"])
