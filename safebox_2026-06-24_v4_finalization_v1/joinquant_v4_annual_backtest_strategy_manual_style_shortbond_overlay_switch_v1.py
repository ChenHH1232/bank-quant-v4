...
def initialize(context):
    ...
    # Manual fundamental style + shortbond overlay switch:
    # - 'balanced_shortbond_overlay'
    # - 'core_level_shadow_shortbond_overlay'
    g.strategy_variant = 'balanced_shortbond_overlay'
    g.allowed_variants = ['balanced_shortbond_overlay', 'core_level_shadow_shortbond_overlay']
    ...
    g.regime_warning_threshold = 0.40
    g.regime_severe_threshold = 0.55
    g.regime_warning_equity_weight = 0.60
    g.regime_severe_equity_weight = 0.40
    g.defensive_asset = '511260.XSHG'
    g.debug = True
...
def build_strategy_variant_config(strategy_variant):
    ...
    if variant == 'balanced_shortbond_overlay':
        return {
            'strategy_variant': 'balanced_shortbond_overlay',
            'manual_style_mode': 'balanced',
        }
    if variant == 'core_level_shadow_shortbond_overlay':
        return {
            'strategy_variant': 'core_level_shadow_shortbond_overlay',
...
def do_rebalance(context):
    ...
    defensive_weight = regime_info['cash_weight']
    target_weight = (equity_weight / float(len(long_list))) if long_list and equity_weight > 0 else 0.0

    if g.debug:
        log.info('========== V4 annual shortbond overlay rebalance ==========')
        log.info('current_date=%s factor_date=%s plan=%s line=%s variant=%s' % (
    ...
    execute_target_weights(context, long_list, target_weight, defensive_weight)
    ...
    g.plan_usage_log.append({
        ...
        'defensive_asset': g.defensive_asset if defensive_weight > 0 else '',
...
def evaluate_fundamental_regime(factor_date):
    ...
    severe_hit_count = sum(value >= g.regime_severe_threshold for value in valid_metric_values)
    warning_hit_count = sum(value >= g.regime_warning_threshold for value in valid_metric_values)

    if severe_hit_count >= 2 or breadth_score >= g.regime_severe_threshold:
        regime_name = 'severe_deterioration'
        equity_weight = g.regime_severe_equity_weight
    elif warning_hit_count >= 2 or breadth_score >= g.regime_warning_threshold:
    ...
    return {
        ...
        'cash_weight': max(0.0, 1.0 - float(equity_weight)),
...
def execute_target_weights(context, long_list, target_weight, defensive_weight):
    target_value_map = {
        stock: context.portfolio.total_value * target_weight
        for stock in long_list
    }
    if defensive_weight > 0:
        target_value_map[g.defensive_asset] = context.portfolio.total_value * defensive_weight
...
def after_trading_end(context):
    ...
    if current_date == datetime(2026, 5, 31).date():
        ...
        for item in g.plan_usage_log:
            log.info(str(item))
        log.info('========== V4 shortbond overlay regime log ==========')
