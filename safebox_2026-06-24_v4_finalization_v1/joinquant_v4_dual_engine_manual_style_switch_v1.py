...
def initialize(context):
    ...
    # Dual-engine initialize switch:
    # - 'balanced_60_40': balanced fundamental line + 60/40 budget
    # - 'core_level_shadow_60_40': core-level shadow fundamental line + 60/40 budget
    g.strategy_variant = 'balanced_60_40'
    g.allowed_variants = ['balanced_60_40', 'core_level_shadow_60_40']
    variant_config = build_strategy_variant_config(g.strategy_variant)

    # Fundamental engine cadence remains aligned with the validated V4 panel.
    g.rebalance_months = [5, 9, 11]
    g.group_count = 5
    g.hold_bucket = 5
    g.strategy_line = 'benchmark'  # 'primary' or 'benchmark'
    g.manual_style_mode = variant_config['manual_style_mode']
    g.fundamental_budget = variant_config['fundamental_budget']
    g.momentum_budget = variant_config['momentum_budget']
    ...
    g.annual_plan_library = build_annual_plan_library(g.manual_style_mode)
...
def build_strategy_variant_config(strategy_variant):
    variant = str(strategy_variant).strip().lower()
    if variant == 'balanced_60_40':
        return {
            'strategy_variant': 'balanced_60_40',
            'manual_style_mode': 'balanced',
            'fundamental_budget': 0.60,
            'momentum_budget': 0.40,
        }
    if variant == 'core_level_shadow_60_40':
        return {
            'strategy_variant': 'core_level_shadow_60_40',
            'manual_style_mode': 'core_level',
            'fundamental_budget': 0.60,
            'momentum_budget': 0.40,
        }
    raise ValueError('unknown g.strategy_variant: %s' % str(strategy_variant))


def build_balanced_annual_plan_library():
    return [
        {
            'plan_id': 'annual_01',
            ...
            'benchmark_factors': [
                factor_spec('bank_indicator__Nonperforming_loan_rate', -1, 0.280029),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.201686),
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.183547),
                factor_spec('indicator__roe', 1, 0.154546),
                factor_spec('indicator__eps', 1, 0.106770),
                factor_spec('bank_indicator__non_performing_loan_provision_coverage', 1, 0.073422),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__Nonperforming_loan_rate', -1, 0.280029),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.201686),
        ...
        },
        {
            ...
            'benchmark_factors': [
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.239831),
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.212160),
                factor_spec('indicator__roe', 1, 0.164195),
                factor_spec('bank_indicator__Nonperforming_loan_rate', -1, 0.159704),
                factor_spec('bank_indicator__non_performing_loan_provision_coverage', 1, 0.130897),
                factor_spec('indicator__eps', 1, 0.093212),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.239831),
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.212160),
        ...
        },
        {
            ...
            'benchmark_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.555317),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.444683),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.555317),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.444683),
        ...
        },
        {
            ...
            'effective_end': None,
            'benchmark_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.408042),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.328576),
                factor_spec('bank_indicator__non_performing_loan_provision_coverage', 1, 0.263382),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.408042),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.328576),
...
def build_core_level_annual_plan_library():
    return [
        {
            'plan_id': 'annual_01',
            'effective_start': datetime(2021, 5, 6).date(),
        ...
        },
        {
            ...
            'benchmark_factors': [
                ...
                factor_spec('bank_indicator__Nonperforming_loan_rate', -1, 0.159704),
                factor_spec('bank_indicator__non_performing_loan_provision_coverage', 1, 0.130897),
                factor_spec('bank_indicator__core_level_capital_adequacy_ratio', 1, 0.146164),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.239831),
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.212160),
        ...
        },
        {
            ...
            'benchmark_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.555317),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.444683),
                factor_spec('bank_indicator__core_level_capital_adequacy_ratio', 1, 0.189957),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.555317),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.444683),
        ...
        },
        {
            ...
            'benchmark_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 1.0),
                factor_spec('bank_indicator__core_level_capital_adequacy_ratio', 1, 0.102698),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 1.0),
                factor_spec('bank_indicator__core_level_capital_adequacy_ratio', 1, 0.102698),
        ...
        },
        {
            ...
            'benchmark_factors': [
                ...
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.328576),
                factor_spec('bank_indicator__non_performing_loan_provision_coverage', 1, 0.263382),
                factor_spec('bank_indicator__core_level_capital_adequacy_ratio', 1, 0.120969),
            ],
            'primary_factors': [
                factor_spec('bank_indicator__capital_adequacy_ratio', 1, 0.408042),
                factor_spec('bank_indicator__deposit_loan_ratio', -1, 0.328576),
...
def build_annual_plan_library(manual_style_mode):
    mode = str(manual_style_mode).strip().lower()
    if mode == 'balanced':
        return build_balanced_annual_plan_library()
    if mode == 'core_level':
        return build_core_level_annual_plan_library()
    raise ValueError('unknown g.manual_style_mode: %s' % str(manual_style_mode))
...
MANUAL_BANK_INDICATOR_CORE_LEVEL_CSV = '''\ncode,source_year,core_level_capital_adequacy_ratio
000001.XSHE,2019,9.11
000001.XSHE,2020,8.69
000001.XSHE,2021,8.6
000001.XSHE,2022,8.64
000001.XSHE,2023,9.22
001227.XSHE,2022,8.47
001227.XSHE,2023,8.41
002142.XSHE,2019,9.62
002142.XSHE,2020,9.52
002142.XSHE,2021,10.16
002142.XSHE,2022,9.75
002142.XSHE,2023,9.64
002807.XSHE,2019,14.16
002807.XSHE,2020,13.34
002807.XSHE,2021,12.96
002807.XSHE,2022,12.77
002807.XSHE,2023,13.1
002839.XSHE,2019,11.02
002839.XSHE,2020,10.35
002839.XSHE,2021,9.82
002839.XSHE,2022,9.36
002839.XSHE,2023,9.76
002936.XSHE,2019,7.98
002936.XSHE,2020,8.92
002936.XSHE,2021,9.49
002936.XSHE,2022,9.29
002936.XSHE,2023,8.9
002948.XSHE,2019,8.36
002948.XSHE,2020,8.35
002948.XSHE,2021,8.38
002948.XSHE,2022,8.75
002948.XSHE,2023,8.42
002958.XSHE,2019,10.48
002958.XSHE,2020,9.73
002958.XSHE,2021,9.62
002958.XSHE,2022,9.77
002958.XSHE,2023,9.91
002966.XSHE,2019,11.3
002966.XSHE,2020,11.26
002966.XSHE,2021,10.37
002966.XSHE,2022,9.63
002966.XSHE,2023,9.38
600000.XSHG,2019,10.26
600000.XSHG,2020,9.51
600000.XSHG,2021,9.4
600000.XSHG,2022,9.19
600000.XSHG,2023,8.97
600015.XSHG,2019,9.25
600015.XSHG,2020,8.79
600015.XSHG,2021,8.78
600015.XSHG,2022,9.24
600015.XSHG,2023,9.16
600016.XSHG,2019,8.89
600016.XSHG,2020,8.51
600016.XSHG,2021,9.04
600016.XSHG,2022,9.17
600016.XSHG,2023,9.28
600036.XSHG,2019,11.95
600036.XSHG,2020,12.29
600036.XSHG,2021,12.66
600036.XSHG,2022,13.68
600036.XSHG,2023,13.73
600908.XSHG,2019,10.2
600908.XSHG,2020,9.03
600908.XSHG,2021,8.74
600908.XSHG,2022,10.97
600908.XSHG,2023,11.27
601939.XSHG,2020,13.62
601939.XSHG,2021,13.59
601939.XSHG,2022,13.69
601939.XSHG,2023,13.15
601963.XSHG,2021,9.36
601963.XSHG,2022,9.52
601963.XSHG,2023,9.78
601988.XSHG,2019,11.3
601988.XSHG,2020,11.28
601988.XSHG,2021,11.3
601988.XSHG,2022,11.84
601988.XSHG,2023,11.63
601997.XSHG,2019,9.39
601997.XSHG,2020,9.3
601997.XSHG,2021,10.62
601997.XSHG,2022,10.95
601997.XSHG,2023,11.84
601998.XSHG,2019,8.69
601998.XSHG,2020,8.74
601998.XSHG,2021,8.85
601998.XSHG,2022,8.74
601998.XSHG,2023,8.99
603323.XSHG,2019,12.17
603323.XSHG,2020,11.38
603323.XSHG,2021,10.72
603323.XSHG,2022,10.17
603323.XSHG,2023,10.19
'''
...
def maybe_rebalance(context):
    ...
    if not is_first_trade_day_of_month(current_date):
        return
    ...
    rebalance_reason = 'fundamental_refresh' if current_date.month in g.rebalance_months else 'monthly_momentum_refresh'
    do_rebalance(context, rebalance_reason)
...
def do_rebalance(context, rebalance_reason):
    ...
    fundamental_weights = build_equal_weight_map(long_list, g.fundamental_budget)
    momentum_df = build_momentum_score_frame(context, factor_date)
    if momentum_df.empty:
        log.info('Momentum score frame is empty on %s' % str(factor_date))
        return
    momentum_weights = build_momentum_engine_weights(momentum_df, g.momentum_budget)
    target_weight_map = combine_engine_weights(fundamental_weights, momentum_weights)
    if len(target_weight_map) == 0:
        log.info('Combined target weight map is empty on %s' % str(factor_date))
        return
    ...
    if g.debug:
        log.info('========== V4 dual-engine manual-style rebalance ==========')
        log.info('current_date=%s factor_date=%s plan=%s line=%s reason=%s variant=%s' % (
        ...
        log.info('budget_summary=%s' % str({
            'fundamental_budget': g.fundamental_budget,
            'momentum_budget': g.momentum_budget,
    ...
    g.latest_rebalance_log = {
        'mode': 'dual_engine_manual_style_switch_v1',
        'strategy_variant': g.strategy_variant,
        'manual_style_mode': g.manual_style_mode,
...
def fetch_bank_indicator_snapshot(stocks, source_year):
    ...
    for stock in stocks:
        ...
        records.append({
            ...
            'bank_indicator__core_level_capital_adequacy_ratio': yearly_data.get('core_level_capital_adequacy_ratio', np.nan),
...
def build_equal_weight_map(long_list, budget):
    if long_list is None or len(long_list) == 0 or budget <= 0:
        return {}
    one_weight = float(budget) / float(len(long_list))
    return {stock: one_weight for stock in long_list}


def build_momentum_score_frame(context, factor_date):
    tradable = get_tradable_stocks(context, g.bank_stocks)
    if len(tradable) == 0:
        return pd.DataFrame()
...
def build_manual_bank_indicator_data():
    ...
    core_level_df = pd.read_csv(StringIO(MANUAL_BANK_INDICATOR_CORE_LEVEL_CSV))
    data = {}
    for _, row in df.iterrows():
        code = row['code']
    ...
    for _, row in core_level_df.iterrows():
        code = row['code']
        source_year = int(row['source_year'])
        data.setdefault(code, {}).setdefault(source_year, {})
        data[code][source_year]['core_level_capital_adequacy_ratio'] = safe_float(
            row['core_level_capital_adequacy_ratio']
        )
    return data
